...
function symbolDatabaseHtml(){
  ...
  return `<section class="symbol-database-shell">
    ...
    <div class="symbol-quick-filters" aria-label="Knitting symbol filters">${["All","Beginner","Intermediate","Advanced","Basic","Increase","Decrease","Lace","Cable","Texture","Colourwork","Short Row","Chart Structure","Construction"].map(value=>`<button class="secondary-button" data-symbol-quick="${value}">${value==="Short Row"?"Short rows":value==="Chart Structure"?"Chart structure":value}</button>`).join("")}</div>
...
function bindSymbolDatabase(){
  ...
  document.querySelectorAll("[data-symbol-quick]").forEach(button=>button.addEventListener("click",()=>{const value=button.dataset.symbolQuick;symbolFilters.craft=value==="All"?"All":"Knitting";symbolFilters.difficulty=["Beginner","Intermediate","Advanced"].includes(value)?value:"All";symbolFilters.category=window.YarnchaSymbolDatabase.categoryOrder.includes(value)?value:"All";renderLibrary();}));
