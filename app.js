...
function symbolDatabaseHtml(){
  ...
  if(currentSymbolId){
    ...
    return `<section class="symbol-detail card">
      ...
      <div class="symbol-detail-hero"><div class="symbol-glyph">${symbolVisualHtml(entry,"detail")}</div><div><p class="eyebrow">${escapeHtml(entry.craft)} · ${escapeHtml(entry.category)} · ${escapeHtml(entry.difficulty)}</p><h2>${escapeHtml(entry.nameEn)}</h2><div class="symbol-abbreviation-block" aria-label="Abbreviations"><h3>Abbreviations</h3><div class="symbol-abbreviation-chips ${entry.craft==="Crochet"?"crochet-abbreviation-chips":""}">${entry.craft==="Crochet"?`<span><small>US</small><strong>${escapeHtml(abbreviations.us||entry.abbreviationUS||"—")}</strong></span><span><small>UK</small><strong>${escapeHtml(abbreviations.uk||entry.abbreviationUK||"—")}</strong></span>`:`<span><small>UK/US</small><strong>${escapeHtml(abbreviations.usUk||"—")}</strong></span>`}<span><small>CN</small><strong>${escapeHtml(abbreviations.cn||"—")}</strong></span><span><small>JP</small><strong>${escapeHtml(abbreviations.jp||"—")}</strong></span></div></div></div></div>
  ...
  const crochetMode=symbolFilters.craft==="Crochet";
  const quickFilters=crochetMode?["All","Beginner","Intermediate","Advanced","Basic","Chain","Slip Stitch","Increase","Decrease","Loop Placement","Post Stitch","Shell","Texture","Lace","Colourwork","Amigurumi","Chart Structure","Finishing"]:["All","Beginner","Intermediate","Advanced","Basic","Increase","Decrease","Lace","Cable","Texture","Colourwork","Short Row","Chart Structure","Construction"];
  return `<section class="symbol-database-shell">
    ...
    <div class="symbol-reference-switch" aria-label="Symbol reference craft"><button class="secondary-button" data-symbol-craft-mode="Knitting">Knitting</button><button class="secondary-button" data-symbol-craft-mode="Crochet">Crochet</button><button class="secondary-button" data-symbol-craft-mode="All">All crafts</button></div>
    ...
    <div class="symbol-quick-filters" aria-label="${crochetMode?"Crochet":"Knitting"} symbol filters">${quickFilters.map(value=>`<button class="secondary-button" data-symbol-quick="${value}">${value==="Short Row"?"Short rows":value==="Chart Structure"?"Chart structure":value==="Slip Stitch"?"Slip stitch":value==="Loop Placement"?"Loop placement":value==="Post Stitch"?"Post stitch":value}</button>`).join("")}</div>
...
function bindSymbolDatabase(){
  ...
  document.querySelectorAll("[data-symbol-quick]").forEach(button=>button.addEventListener("click",()=>{const value=button.dataset.symbolQuick,currentCraft=symbolFilters.craft;symbolFilters.craft=["Knitting","Crochet"].includes(currentCraft)?currentCraft:(value==="All"?"All":"Knitting");symbolFilters.difficulty=["Beginner","Intermediate","Advanced"].includes(value)?value:"All";symbolFilters.category=window.YarnchaSymbolDatabase.categoryOrder.includes(value)?value:"All";renderLibrary();}));
  document.querySelectorAll("[data-symbol-craft-mode]").forEach(button=>button.addEventListener("click",()=>{symbolFilters.craft=button.dataset.symbolCraftMode;symbolFilters.category="All";symbolFilters.difficulty="All";symbolFilters.search="";renderLibrary();}));
