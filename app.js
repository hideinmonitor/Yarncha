...
const themePresets=[
  {id:"creamy-vanilla",name:"The defaults",badge:"Original",description:"The original June 20 Yarncha palette: warm cream, paper cards, sage controls, and clay accents.",recommended:true,canDelete:false,primary:"#B7785F",secondary:"#E7E9DC",accent:"#B7785F",background:"#F6F0E6",surface:"#FFFAF3",card:"#FFFAF3",border:"#E3D8CA",text:"#463D35",textSecondary:"#7D7268",button:"#5F6958",buttonText:"#FFFDF8",secondaryButton:"#E7E9DC",success:"#3D6947",warning:"#986422",danger:"#A44E43",highlight:"#C4A269",link:"#5F6958",colors:{background:"#F6F0E6",surface:"#FFFAF3",surfaceSecondary:"#E7E9DC",primary:"#B7785F",primaryHover:"#4F5949",secondary:"#E7E9DC",accent:"#B7785F",accentSoft:"#C4A269",border:"#E3D8CA",divider:"#E3D8CA",text:"#463D35",textSecondary:"#7D7268",success:"#3D6947",warning:"#986422",error:"#A44E43",chartCurrentRow:"#C4A269",chartCompleted:"#E7E9DC",annotationPen:"#B7785F",annotationHighlight:"rgba(196,162,105,0.34)",voiceListening:"#B7785F",shadow:"rgba(86,68,52,0.10)"},dark:{background:"#1A120C",surface:"#2B1C12",card:"#352314",text:"#FFF1D7",textSecondary:"#D3C09A",button:"#FCC277",buttonText:"#241407",secondary:"#4B3D24",accent:"#C96C23",border:"#6F5730",success:"#B2B978",warning:"#FCC277",danger:"#E28A62",highlight:"#FCC277",link:"#FCC277"}},
...
const designStyles=[
  {id:"original-classic",name:"Original Classic",tag:"Recommended",desc:"The balanced, calm, handmade style of the original Yarncha."},
...
let symbolFilters = { search:"", craft:"All", category:"All", referenceCategory:"All", passType:"All", difficulty:"All", terminology:"All", verification:"All", sort:"Default" };
...
async function dataUrlToBlob(dataUrl){
  ...
  if(!match)throw new Error("Unsafe backup asset data.");
...
function normalizeThemeName(name){
  ...
  if(key.includes("creamy-vanilla")||key.includes("creamy vanilla")||key.includes("the defaults")||key.includes("original yarncha")||key.includes("vintage-paper")||key.includes("corner-of-light")||key.includes("corner of light"))return "creamy-vanilla";
  return { "soft-yarn":"creamy-vanilla", "cocoa-beige":"creamy-vanilla", sakura:"sakura-milk", forest:"matcha-grove", "matcha-latte":"matcha-grove", "peach-apricot":"sky-blessing", "soft-lavender":"lavender-twilight" }[key] || (themePresets.some(t=>t.id===key)?key:"creamy-vanilla");
}
...
function loadState() {
  try {
    ...
    const savedMaterials=Array.isArray(saved.yarnMaterials)?saved.yarnMaterials:[];
    const savedMaterialsById=new Map(savedMaterials.filter(item=>item?.id).map(item=>[item.id,item]));
    const builtInMaterials=defaultYarnMaterials().map(item=>({...item,...(savedMaterialsById.get(item.id)||{}),builtIn:true}));
    const customMaterials=savedMaterials.filter(item=>!item.builtIn&&!builtInMaterials.some(defaultItem=>defaultItem.id===item.id));
    merged.yarnMaterials=[...builtInMaterials,...customMaterials];
...
async function manualSave(context="Project"){
  ...
  try{
    ...
    if(!hadPending&&!cloud?.user){
      updateSaveStatus(`✓ Saved on this device · ${formatSavedTime(state.lastSavedAt||new Date())}`,"saved");
      toast("Everything is already saved.");
      return false;
    }
    if(hadPending)await saveState();
    if(cloud?.user&&window.YarnchaCloud?.syncNow){
      updateSaveStatus("Saving... syncing cloud","saving");
      const result=await window.YarnchaCloud.syncNow("manual-save");
      const message=window.YarnchaSyncProtocol.describeSyncResult(result,context);
      updateSaveStatus(message.statusText,message.tone);
      toast(message.toast);
    }else{
      updateSaveStatus(`✓ Saved on this device · ${formatSavedTime(state.lastSavedAt||new Date())}`,"saved");
      toast(`✓ ${context} saved`);
...
const uiIconPaths={
  ...
  account:'<circle cx="12" cy="8" r="3.25"></circle><path d="M5.5 19c.7-3.4 3-5.2 6.5-5.2s5.8 1.8 6.5 5.2"></path>'
};
...
async function showView(name) {
  ...
  try{
    ...
    document.querySelectorAll(".nav-item").forEach(item => {
      ...
      if(item.dataset.view){if(navId===requested)item.setAttribute("aria-current","page");else item.removeAttribute("aria-current");}
    });
    document.querySelectorAll(".side-project").forEach(item=>{
      if(route.viewId==="project-detail"&&String(item.dataset.project)===String(currentProjectId))item.setAttribute("aria-current","page");
      else item.removeAttribute("aria-current");
    });
    document.getElementById("breadcrumb").textContent = route.label;
    document.getElementById("header-context").textContent = route.viewId === "project-detail" ? "Project" : "Yarncha";
    document.querySelector(".topbar")?.classList.toggle("project-context",route.viewId==="project-detail");
...
function handleAppShellClick(e){
  ...
  const learningPath=e.target.closest("[data-wiki-learning-path]");if(learningPath){e.preventDefault();openLibraryLearningPath(learningPath.dataset.wikiLearningPath);}
}
...
function renderSidebar() {
  document.getElementById("sidebar-project-list").innerHTML = state.projects.slice(0,5).map(p =>
    `<button class="side-project ${String(p.id)===String(currentProjectId)?"active":""}" data-project="${p.id}" aria-label="Open ${escapeHtml(p.name)}"><span class="project-dot" style="background:${p.color}"></span><span>${escapeHtml(p.name)}</span></button>`
  ).join("");
  renderAccountControl();
}

function accountShellDetails(){
  const status=window.YarnchaCloud?.status;
  const email=status?.user?.email||"";
  if(email)return{label:email,meta:status.lastError?"Cloud sync needs attention":"Cloud sync connected",initial:email[0].toUpperCase()};
  const localLabel=state.account?.email||"";
  return{label:"Log in",meta:status?.configured?"Sync across devices":localLabel||"Saved on this device",initial:"Y"};
}
function renderAccountControl(){
  const control=document.getElementById("account-button");if(!control)return;
  const details=accountShellDetails();
  document.getElementById("account-label").textContent=details.label;
  document.getElementById("account-meta").textContent=details.meta;
  document.getElementById("account-avatar").textContent=details.initial;
  control.setAttribute("aria-label",details.label==="Log in"?"Log in or create an account":`Manage account for ${details.label}`);
  if(!document.getElementById("modal-backdrop")?.classList.contains("open"))control.setAttribute("aria-expanded","false");
}
function openSettingsSection(sectionId=""){
  closeModal(true);
  return showView("settings").then(()=>{
    if(!sectionId)return;
...
function openPhoneHelp(){
  openModal(`<p class="eyebrow">DEVICE ACCESS</p><h2>Open Yarncha on your phone</h2><p><strong>On this Mac:</strong> open <code>http://localhost:4183/</code> while the Yarncha preview server is running.</p><p><strong>On your phone:</strong> localhost points to the phone itself, so it will not open Yarncha. Connect the phone and Mac to the same Wi-Fi, then open <code>http://MAC_LOCAL_IP:4183/</code> using the Mac's local network address.</p><p><strong>If it still does not connect:</strong> allow incoming connections through the Mac firewall and try another available port such as 4184 or 5173.</p><p><strong>Deployed website:</strong> once Yarncha is deployed, use its hosted address instead of a local network address.</p><div class="privacy-note">Local browser storage does not automatically sync between devices or browsers. Export a backup before switching browser, clearing data, or moving device.</div><div class="modal-actions"><button class="primary-button" data-close-modal>Close</button></div>`);
}
function openMobileMoreMenu(){
  const details=accountShellDetails();
  openModal(`<p class="eyebrow">YARNCHA</p><h2>More</h2><nav class="mobile-more-menu" aria-label="More Yarncha sections"><button type="button" data-more-view="market">${uiIcon("storage","ui-icon")}<span><strong>Yarn Stash</strong><small>Inventory and shopping</small></span></button><button type="button" data-more-view="library">${uiIcon("book","ui-icon")}<span><strong>Library</strong><small>Patterns, tutorials and ideas</small></span></button><button type="button" id="mobile-more-account">${uiIcon("account","ui-icon")}<span><strong>${escapeHtml(details.label)}</strong><small>${escapeHtml(details.meta)}</small></span></button><button type="button" data-more-settings="settings-appearance">${uiIcon("appearance","ui-icon")}<span><strong>Appearance</strong><small>Theme, mode and language</small></span></button><button type="button" data-more-settings>${uiIcon("preferences","ui-icon")}<span><strong>Settings</strong><small>Preferences, sync and backup</small></span></button></nav>`);
  document.querySelectorAll("[data-more-view]").forEach(button=>button.onclick=()=>{closeModal(true);showView(button.dataset.moreView);});
  document.querySelectorAll("[data-more-settings]").forEach(button=>button.onclick=()=>openSettingsSection(button.dataset.moreSettings||""));
  document.getElementById("mobile-more-account").onclick=()=>{closeModal(true);openAccountModal();};
}
...
function renderToday() {
  ...
  if (!p) {
    host.innerHTML = `<div class="empty-state"><h3>Your first project starts here</h3><p>Choose New project from the sidebar to begin.</p></div>`;
    return;
  }
...
function renderProjects() {
  ...
  </button>`).join("") || `<div class="empty-state"><h3>Your first project starts here</h3><p>Keep your pattern, progress and notes together.</p><button class="primary-button" id="empty-new-project">New project</button></div>`;
  ...
  document.getElementById("project-detail").innerHTML = `
    <div class="project-mobile-shell ${p.readingMode?"is-reading":""}">
      <div class="detail-head project-header">
        ...
        <div class="detail-actions project-actions"><button class="project-action-button ghost" id="edit-project-name">Edit project</button></div>
      </div>
...
function projectTrackHtml(p){
  ...
  return `<div class="track-layout">
    <div class="counter-card card hero-counter">
      ...
      <div class="counter-controls counter-controls-large main-row-controls"><button data-counter="-1" aria-label="Previous row">−</button><button class="next button-primary" data-counter="1">Next row</button></div>
      <div class="counter-context-actions"><button class="button-secondary" id="speak-row" type="button">Read current row</button><button class="button-secondary contextual-voice-button" data-voice-project type="button">${uiIcon("voice","voice-button-icon")}<span>Voice row controls</span></button></div>
      ...
      <div class="counter-menu button-group"><button class="button-secondary" id="edit-project-rows">Planned rows</button><button class="button-ghost" id="reset-main">Reset rows</button></div>
      ${unifiedRepeatCountersHtml(p)}
    </div>
    <div class="track-side">
      ${rowRemindersPanelHtml(p)}
      <div class="notes-card card"><div class="card-header"><div><h3><label for="project-notes">Project notes</label></h3><span class="manual-save-status" data-save-indicator>✓ Saved</span></div><div class="button-row compact-save-actions"><button class="mini-button button-secondary" id="add-marker">+ Marker</button></div></div>
  ...
  return `<div class="project-workspace-page chart-mode ${chartMode==="flow"?"flow-workflow":""} ${p.readingMode?"reading-mode":""}">
    ...
    <div class="reading-counter row-counter-card card workspace-card">
      ...
      <div class="row-stepper">
        ...
        <button class="button-icon contextual-voice-button inline-voice-button" data-voice-project aria-label="Voice row controls" title="Voice row controls">${uiIcon("voice","voice-button-icon")}<span>Voice</span></button>
      </div>
...
function friendlyChartBetaHtml(p){
  ...
  const noticeMessages=[
    !hasChart?"Add a chart so Yarncha can guide you row by row.":hasUnclear?"Please review the unclear symbols before Yarncha reads them as instructions.":"",
    ...plan.warnings,
    plan.estimateOnly&&!plan.warnings.length?"Add your gauge for a more accurate finished-size estimate.":""
  ].filter(Boolean);
  const noticeTitle=!hasChart?"Chart needed":hasUnclear?"Quick check needed":plan.estimateOnly?"Estimate only":"Ready to follow";
  return `<div class="chart-analysis ai-beta-safe flow-reader-card card workspace-card" id="flow-ai-reader">
    ...
    <section class="flow-project-context"><div><p class="eyebrow">YOUR PROJECT</p><h4>${escapeHtml(p.name||"Current project")}</h4><p>${escapeHtml(setup.craft||p.type||"Pattern")} · Row ${p.row}${p.chartRows||p.totalRows?` of ${p.chartRows||p.totalRows}`:""}</p></div><div class="flow-project-context-actions"><span class="flow-chart-status ${ready?"ready":"almost"}">${ready?"Chart ready":"Setup needed"}</span><button class="mini-button" id="start-flow-mode" type="button">Start Flow Mode</button></div></section>
    ${noticeMessages.length?`<div class="flow-notice ${ready&&!plan.estimateOnly?"ready":"almost"}" role="status"><span aria-hidden="true">${ready&&!plan.estimateOnly?"✓":"!"}</span><div><strong>${escapeHtml(noticeTitle)}</strong>${noticeMessages.map(message=>`<p>${escapeHtml(message)}</p>`).join("")}</div></div>`:""}
    <section class="flow-section flow-reading-mode-panel"><p class="eyebrow">READING PROGRESS</p>
      <div class="flow-current-row-card"><span class="flow-current-row-label">I'm currently on</span><output id="flow-current-row" aria-live="polite">Row <strong>${p.row}</strong></output><p>${escapeHtml(rowInstruction)}</p><small>${escapeHtml(flowCurrentStitchStep(p,setup))}</small></div>
      ...
      <p class="flow-next-line"><strong>Next:</strong> ${flowReadingSummary(p,reader)}</p>
    </section>
    <section class="flow-section flow-pattern-guidance"><div class="flow-section-heading"><p class="eyebrow">PATTERN GUIDANCE</p><h4>Keep the chart easy to follow</h4></div>
      <div class="flow-guidance-controls"><label class="flow-direction-field">Pattern language<select id="flow-pattern-language">${options(["abbreviations","full"],setup.patternLanguage,{abbreviations:"Abbreviations",full:"Full wording"})}</select><small>Show short notation or spell each stitch out.</small></label>
      <label class="flow-direction-field">Reading direction<select id="flow-row-direction">${options(["left-to-right","right-to-left","alternating-rs-ws"],reader.rowDirection,directionLabels)}</select><small>Alternating rows is common when right-side and wrong-side rows switch direction.</small></label></div>
    ...
    <section class="flow-section flow-setup-results-panel">
      ${resultSummaryHtml("Project Setup Summary",flowCalculationItems(plan),"flow-calculation-summary result-summary-plain")}
    ...
    <section class="flow-section flow-read-aloud">
    ...
    <details class="flow-section flow-advanced-tools"><summary>Advanced tools</summary>
      <div class="flow-reader-grid">
        <section class="flow-reader-panel"><h4>Chart setup</h4><div class="form-grid compact-form">
          <div class="field"><label for="flow-chart-type">Craft type</label><select id="flow-chart-type">${options(["Knitting","Crochet","Tunisian Crochet","Unknown"],reader.chartType)}</select></div>
          <div class="field"><label for="flow-reading-direction">Starting direction</label><select id="flow-reading-direction">${options(["right-to-left","left-to-right","alternating-rs-ws","round","auto"],reader.readingDirection,{...directionLabels,round:"In the round",auto:"Let Yarncha choose"})}</select></div>
          <div class="field"><label for="flow-crop-x">Left edge</label><input id="flow-crop-x" type="number" min="0" max="100" value="${reader.crop.x}"></div>
          <div class="field"><label for="flow-crop-y">Top edge</label><input id="flow-crop-y" type="number" min="0" max="100" value="${reader.crop.y}"></div>
          <div class="field"><label for="flow-crop-width">Chart width</label><input id="flow-crop-width" type="number" min="1" max="100" value="${reader.crop.width}"></div>
          <div class="field"><label for="flow-crop-height">Chart height</label><input id="flow-crop-height" type="number" min="1" max="100" value="${reader.crop.height}"></div>
        ...
        <section class="flow-reader-panel"><h4>Fine-tune rows</h4><div class="form-grid compact-form">
          <div class="field"><label for="flow-grid-rows">Rows on chart</label><input id="flow-grid-rows" type="number" min="0" value="${reader.grid.rows||""}" placeholder="Optional"></div>
          <div class="field"><label for="flow-grid-columns">Stitches across</label><input id="flow-grid-columns" type="number" min="0" value="${reader.grid.columns||""}" placeholder="Optional"></div>
          <div class="field"><label for="flow-grid-x">Guide left</label><input id="flow-grid-x" type="number" min="0" max="100" value="${reader.grid.x}"></div>
          <div class="field"><label for="flow-grid-y">Guide top</label><input id="flow-grid-y" type="number" min="0" max="100" value="${reader.grid.y}"></div>
          <div class="field"><label for="flow-grid-width">Guide width</label><input id="flow-grid-width" type="number" min="1" max="100" value="${reader.grid.width}"></div>
          <div class="field"><label for="flow-grid-height">Guide height</label><input id="flow-grid-height" type="number" min="1" max="100" value="${reader.grid.height}"></div>
...
function flowField(id,label,value="",placeholder="Optional",full=false){
  return `<div class="field ${full?"full":""}"><label for="${id}">${escapeHtml(label)}</label><input id="${id}" value="${escapeHtml(value||"")}" placeholder="${escapeHtml(placeholder)}"></div>`;
}
function flowSelect(id,label,values,current,options,full=false,help=""){
  return `<div class="field ${full?"full":""}"><label for="${id}">${escapeHtml(label)}</label><select id="${id}">${options(values,current)}</select>${help?`<small>${escapeHtml(help)}</small>`:""}</div>`;
}
  ...
  return `<section class="card mobile-card fit-check-card">
    ...
    <div class="fit-check-grid">
      <div class="fit-check-form form-grid compact-form">
        <div class="field"><label for="fit-body-chest">Body chest / main measurement cm</label><input id="fit-body-chest" type="number" min="0" step=".5" value="${value("bodyChest",check.bodyChest||"")}" placeholder="e.g. 100"></div>
        <div class="field"><label for="fit-finished-chest">Finished width / chest cm</label><input id="fit-finished-chest" type="number" min="0" step=".5" value="${value("finishedChest",check.finishedChest||"")}" placeholder="e.g. 108"></div>
        <div class="field"><label for="fit-intended-ease">Intended ease cm</label><input id="fit-intended-ease" type="number" step=".5" value="${value("intendedEase",check.intendedEase)}" placeholder="e.g. 8"></div>
        <div class="field"><label for="fit-target-length">Target length cm</label><input id="fit-target-length" type="number" min="0" step=".5" value="${value("targetLength",check.targetLength||"")}" placeholder="e.g. 58"></div>
        <div class="field"><label for="fit-current-length">Current length cm</label><input id="fit-current-length" type="number" min="0" step=".5" value="${value("currentLength",check.currentLength||"")}" placeholder="Measure now"></div>
        <div class="field"><label for="fit-feeling">Fit feeling</label><select id="fit-feeling">${["Not checked yet","Too tight","Feels right","Too loose","Length unsure"].map(option=>`<option ${check.saved.feeling===option?"selected":""}>${option}</option>`).join("")}</select></div>
        <div class="field full"><label for="fit-notes">Try-on notes</label><textarea id="fit-notes" rows="3" placeholder="Armhole, sleeve, length, shoulder, bust, waist...">${escapeHtml(check.saved.notes||"")}</textarea></div>
      </div>
...
function projectSetupPanelHtml(p,{context="project",embedded=false}={}){
  ...
  const select=(name,values,current=setup[name])=>`<select id="${context}-setup-${name}" data-project-setup-field="${name}">${values.map(option=>`<option value="${escapeHtml(option)}" ${current===option?"selected":""}>${escapeHtml(option)}</option>`).join("")}</select>`;
  const field=(name,label,placeholder="",type="text",full=false)=>`<div class="field ${full?"full":""}"><label for="${context}-setup-${name}">${escapeHtml(label)}</label><input id="${context}-setup-${name}" data-project-setup-field="${name}" type="${type}" value="${value(name)}" placeholder="${escapeHtml(placeholder)}"></div>`;
  const textArea=(name,label,placeholder="",rows=3)=>`<div class="field full"><label for="${context}-setup-${name}">${escapeHtml(label)}</label><textarea id="${context}-setup-${name}" data-project-setup-field="${name}" rows="${rows}" placeholder="${escapeHtml(placeholder)}">${value(name)}</textarea></div>`;
  ...
  return `<section class="${embedded?"project-setup-panel embedded":"card mobile-card project-setup-panel"}" data-project-setup-form="${escapeHtml(context)}">
    ...
    <div class="project-setup-subsection">
      ...
      <div class="project-setup-grid form-grid compact-form">
        <div class="field"><label for="${context}-setup-craftType">Craft</label>${select("craftType",["Knitting","Crochet","Tunisian Crochet"],setup.craftType||setup.craft)}</div>
        <div class="field"><label for="${context}-setup-projectType">Project type</label>${select("projectType",["Scarf","Socks","Hat / Beanie","Shawl","Bag","Blanket","Amigurumi","Top","Cardigan","Jumper / Sweater","Vest","Dress","Other"])}</div>
    ...
    <div class="project-setup-subsection fit-check-card">
      ...
      <div class="fit-check-grid">
        <div class="project-setup-grid form-grid compact-form">
          ...
          <div class="field"><label for="${context}-setup-fitFeeling">Fit feeling</label>${select("fitFeeling",["Not checked yet","Too tight","Feels right","Too loose","Length unsure"])}</div>
    ...
    <div class="project-setup-subsection">
      ...
      <div class="project-setup-grid form-grid compact-form">
        ...
        <div class="field"><label for="${context}-setup-materialStatus">Bought / owned status</label>${select("materialStatus",["Owned","Need to buy","Partly owned","Wishlist"])}</div>
      </div>
    </div>
...
function flowProjectSetupSummaryHtml(p){
  ...
  return collapsibleSectionHtml({eyebrow:"PROJECT DETAILS",title:"Project guide",description:"Saved craft, project type, yarn, hook/needle, start, and rows for Flow Mode and project tools.",rightMeta:summary,className:"flow-section flow-setup-panel flow-project-setup-card",children:`<div id="flow-project-setup-editor">${projectSetupPanelHtml(p,{context:"flow",embedded:true})}</div>`});
}
...
const projectContextService={
  ...
  getCurrentProjectContext(){
    ...
    const patternSource=normalizePatternSource(project?.patternSource||{},project||{});
    const reviewedChartText=String(patternSource.userCorrectedText||project?.pdfReference||patternSource.extractedText||"").trim();
    return {
      projectId:project?.id||"",projectName:project?.name||"",craftType:normalizeAssistantCraft(setup.craft||chart.chartType||project?.type),projectType:setup.projectType||project?.projectKind||project?.type||"",
...
const castOnAssistantService={
  answer(question="",context={}){
    const q=String(question).toLowerCase();
    let recommendation="Long-Tail Cast-On",reason="It is a neat, strong, moderately stretchy general-purpose start.";
    if(/mid(dle)?[- ]?row|add(ing)? stitches|buttonhole/.test(q)){recommendation="Backward Loop Cast-On";reason="It is quick and practical for adding a small number of stitches within the work.";}
    else if(/toe[- ]?up|sock toe|seamless toe/.test(q)){recommendation=/turkish/.test(q)?"Turkish Cast-On":"Judy’s Magic Cast-On";reason="It creates a seamless closed start for a toe-up sock.";}
    else if(/sock|hat|cuff|mitten|stretch/.test(q)){recommendation="German Twisted Cast-On / Old Norwegian Cast-On";reason="It gives cuffs more stretch and strength than a tight standard Long-Tail edge.";}
    else if(/1x1|rib(bing)?|sweater hem|polished/.test(q)){recommendation="Tubular Cast-On";reason="It is stretchy and flows cleanly into 1x1 ribbing for a polished edge.";}
    else if(/temporary|live stitches|folded hem|graft|both directions|provisional/.test(q)){recommendation="Provisional Cast-On";reason="It can be removed later to reveal live stitches for grafting or knitting in another direction.";}
    else if(/decorative|picot|baby|pretty/.test(q)){recommendation="Picot Cast-On";reason="It makes the cast-on edge a visible decorative feature.";}
    else if(/rounded|i.?cord|blanket edge|bag edge/.test(q)){recommendation="I-Cord Cast-On";reason="It creates a polished, rounded, structured decorative edge.";}
    else if(/firm|button band|structure/.test(q)){recommendation="Cable Cast-On";reason="It creates a neat, firm, stable edge.";}
    else if(/easiest|beginner|simple/.test(q)){recommendation="Knitted Cast-On";reason="Its movement is easy to understand once you can make a knit stitch, and it forms a stable edge.";}
    const tooTight=/too tight|hard to knit|does not stretch/.test(q),tooLoose=/too loose|large gaps|messy|unstable/.test(q),twisted=/twist|joining in the round/.test(q),tail=/tail|ran out|enough yarn/.test(q);
    const warning=tooTight?"The existing edge is too tight: test stretch now and restart with a larger needle or a stretchier method if needed.":tooLoose?"The existing edge is loose: avoid Backward Loop for a long project edge and choose Long-Tail, Knitted, or Cable instead.":twisted?"Lay every stitch flat before joining; if the round is already twisted, undo and rejoin or restart.":tail?"A long-tail method can run out of tail; leave extra yarn or use the two-strand variation.":"Keep the loops relaxed, count before Row 1, and check that stitches are not twisted before joining in the round.";
    const entry=libraryWikiEntries.find(item=>item.title===recommendation)||libraryWikiEntryById("cast-on-methods");
    const missing=[!context.projectType&&"project type",!context.edgePurpose&&"edge purpose",context.needsStretch===undefined&&"whether the edge needs stretch",!context.yarnWeight&&"yarn type",!context.toolSize&&"needle size",!context.workingMethod&&"whether you are working flat or in the round",!context.skillLevel&&"skill level"].filter(Boolean);
    return {questionType:"castOnHelp",quickAnswer:`Recommended: ${recommendation}. ${reason}`,whatToDoNow:warning,steps:["Confirm the project type and purpose of the edge.","Check whether the opening must stretch and whether you are working flat or in the round.","Make a short test edge using the project yarn and needle size.","Test the edge, count the stitches, and check for twists before continuing."],checkBeforeContinuing:["The first row is easy to enter.","The edge stretches or stays firm as the project requires.","The stitch count is correct and the round is not twisted."],commonMistakes:["Choosing a firm edge for a stretchy opening.","Pulling each loop tight against the needle.","Not leaving enough tail.","Joining a twisted cast-on in the round."],relatedTechniques:["cast-on methods",recommendation],libraryLinks:[{title:"Cast-on Methods",target:"library:cast-on-methods"},{title:entry.title,target:`library:${entry.id}`}],relatedTools:castOnRelatedTools,approvedLibraryEntries:["Cast-on Methods",entry.title],missingInformation:missing,assumptions:["Giving knitting cast-on guidance.",context.projectType?`Using project type: ${context.projectType}.`:"No project type was provided, so the recommendation is general."],confidence:missing.length?"general":"medium",sourceType:"local-library-rule-based"};
  }
};
const yarnchaAssistantService={
  classifyQuestion(question=""){
    const lower=String(question||"").toLowerCase();
    if(/cast[- ]?on|casting on|join(ing)? in the round/.test(lower))return "castOnHelp";
  ...
  async askYarnchaAssistant({question="",projectContext={},skillLevel="beginner",craftType="knitting"}={}){
    ...
    const diagnostic=this.diagnosticContext(context,q);
    diagnostic.missingInformation=[...(base.missingInformation||[]),...diagnostic.missingInformation].filter((item,index,all)=>all.indexOf(item)===index).slice(0,10);
    diagnostic.assumptions=[...(base.assumptions||[]),...diagnostic.assumptions].filter((item,index,all)=>all.indexOf(item)===index);
...
function projectToolsHtml(p) {
  ...
  return `<div class="project-tools card toolkit-card compact-toolkit"><div class="section-heading" style="margin:0"><div><p class="eyebrow">PROJECT TOOLKIT</p><h2>Choose a focused tool</h2><p class="muted-copy">Showing ${escapeHtml(normalizeProjectType(p.type).toLowerCase())} tools plus shared tools. Budget stays in Buy List / Budget.</p><p class="unit-preference-note">Preferred units: ${escapeHtml(unitSystemLabel())}</p><p class="shared-tool-note">Shared setup: ${escapeHtml(sharedSetup.projectType)} · ${escapeHtml(sharedSetup.patternGauge||"add gauge")} · ${escapeHtml(sharedPlan.widthCm?`${sharedPlan.widthCm} cm wide`:"add size")}</p></div><label class="link-toggle"><input id="link-project-tools" type="checkbox" ${linked ? "checked" : ""}> Save results to this project</label></div>
    <div class="toolkit-selector-panel">
      <div class="field"><label for="project-tool-category">Category</label><select id="project-tool-category">${categories.map(c=>`<option value="${c.id}" ${c.id===categoryId?"selected":""}>${escapeHtml(c.title)}</option>`).join("")}</select><small>${escapeHtml(cat.desc)}</small></div>
      <div class="field"><label for="project-tool-picker">Tool</label><select id="project-tool-picker">${toolOptions.map(t=>`<option value="${t.id}" ${(categoryId==="rendering"&&t.id==="rendering-studio")||t.id===selected?"selected":""}>${escapeHtml(t.name)}</option>`).join("")}</select><small>${escapeHtml(projectToolModeCopy(p,def))}</small></div>
...
function studioSharedPlanningHtml(studio){
  ...
  return `<div class="studio-shared-inputs"><h4>Gauge and yarn estimate</h4><div class="form-grid"><div class="field"><label for="studio-setting-stitchGauge">Stitch gauge per 10 cm</label><input id="studio-setting-stitchGauge" data-studio-setting="stitchGauge" type="number" min="0" step=".1" value="${s.stitchGauge||""}" placeholder="Optional"></div><div class="field"><label for="studio-setting-rowGauge">Row gauge per 10 cm</label><input id="studio-setting-rowGauge" data-studio-setting="rowGauge" type="number" min="0" step=".1" value="${s.rowGauge||""}" placeholder="Optional"></div><div class="field"><label for="studio-setting-totalGrams">Estimated total yarn grams</label><input id="studio-setting-totalGrams" data-studio-setting="totalGrams" type="number" min="0" step=".1" value="${s.totalGrams||""}" placeholder="Optional"></div></div></div>`;
}
function studioSettingsHtml(studio){
  const s=studio.settings;
  const shared=studioSharedPlanningHtml(studio);
  if(studio.mode==="Grid Mode")return `<div class="studio-settings-panel"><div class="studio-form-grid"><div class="field"><label for="studio-columns">Number of columns</label><input id="studio-columns" data-studio-setting="columns" type="number" min="2" value="${s.columns}"></div><div class="field"><label for="studio-grid-rows">Number of rows</label><input id="studio-grid-rows" data-studio-setting="gridRows" type="number" min="2" value="${s.gridRows}"></div><div class="field"><label for="studio-cell-size">Cell size</label><input id="studio-cell-size" data-studio-setting="cellSize" type="number" min="10" max="34" value="${s.cellSize}"></div><div class="field"><label for="studio-active-colour">Paint colour</label><select id="studio-active-colour">${studioSwatchOptions(studio)}</select></div></div><div class="studio-toolbar" aria-label="Grid Mode tools"><button class="mini-button ${studio.paintTool==="brush"?"active":""}" data-studio-paint-tool="brush">Click cell to change colour</button><button class="mini-button ${studio.paintTool==="bucket"?"active":""}" data-studio-paint-tool="bucket">Fill bucket</button><button class="mini-button ${studio.paintTool==="eraser"?"active":""}" data-studio-paint-tool="eraser">Eraser</button><button class="mini-button" data-studio-action="replace-colour">Replace colour</button><button class="mini-button" data-studio-action="export-grid">Export chart</button><button class="mini-button" data-studio-action="save-grid">Save grid to project</button><label><input type="checkbox" data-studio-check="mirrorH" ${s.mirrorH?"checked":""}> Mirror horizontally</label><label><input type="checkbox" data-studio-check="mirrorV" ${s.mirrorV?"checked":""}> Mirror vertically</label></div>${shared}</div>`;
  if(studio.mode==="Granny Square Mode")return `<div class="studio-settings-panel"><div class="form-grid"><div class="field"><label for="studio-setting-centreColor">Centre colour</label><select id="studio-setting-centreColor" data-studio-setting="centreColor">${studio.colors.map((c,i)=>`<option value="${i}" ${Number(s.centreColor)===i?"selected":""}>${escapeHtml(c.name)}</option>`).join("")}</select></div><div class="field"><label for="studio-setting-borderIndex">Border colour</label><select id="studio-setting-borderIndex" data-studio-setting="borderIndex">${studio.colors.map((c,i)=>`<option value="${i}" ${Number(s.borderIndex)===i?"selected":""}>${escapeHtml(c.name)}</option>`).join("")}</select></div><div class="field"><label for="studio-setting-rounds">Number of rounds</label><input id="studio-setting-rounds" data-studio-setting="rounds" type="number" min="1" max="12" value="${s.rounds}"></div><div class="field"><label for="studio-setting-numberSquares">Number of squares</label><input id="studio-setting-numberSquares" data-studio-setting="numberSquares" type="number" min="1" value="${s.numberSquares}"></div><div class="field"><label for="studio-setting-gramsPerSquare">Yarn grams per square</label><input id="studio-setting-gramsPerSquare" data-studio-setting="gramsPerSquare" type="number" min="0" step=".1" value="${s.gramsPerSquare||""}" placeholder="Optional"></div><label class="check-row"><input type="checkbox" data-studio-check="repeatRounds" ${s.repeatRounds?"checked":""}><span>Repeat colour sequence</span></label></div><p class="muted-copy">Outer rounds use more yarn, so usage uses weighted round units.</p>${shared}</div>`;
  if(studio.mode==="Checkerboard Mode")return `<div class="studio-settings-panel"><div class="form-grid"><div class="field"><label for="studio-setting-columns">Number of columns</label><input id="studio-setting-columns" data-studio-setting="columns" type="number" min="1" value="${s.columns}"></div><div class="field"><label for="studio-setting-gridRows">Number of rows</label><input id="studio-setting-gridRows" data-studio-setting="gridRows" type="number" min="1" value="${s.gridRows}"></div><div class="field"><label for="studio-setting-blockSize">Block size</label><input id="studio-setting-blockSize" data-studio-setting="blockSize" type="number" min="1" value="${s.blockSize}"></div></div><p class="muted-copy">Uses the first two colours as Colour A and Colour B; extra colours can join the alternating order.</p>${shared}</div>`;
  if(studio.mode==="Gradient Mode")return `<div class="studio-settings-panel"><div class="form-grid"><div class="field"><label for="studio-setting-gradientSteps">Number of steps</label><input id="studio-setting-gradientSteps" data-studio-setting="gradientSteps" type="number" min="2" value="${s.gradientSteps}"></div><div class="field"><label for="studio-setting-rows">Total rows</label><input id="studio-setting-rows" data-studio-setting="rows" type="number" min="2" value="${s.rows}"></div><div class="field"><label for="studio-setting-rowsPerStripe">Rows per step</label><input id="studio-setting-rowsPerStripe" data-studio-setting="rowsPerStripe" type="number" min="1" value="${s.rowsPerStripe}"></div><div class="field"><label for="studio-setting-gradientDirection">Gradient direction</label><select id="studio-setting-gradientDirection" data-studio-setting="gradientDirection">${["Top to bottom","Left to right","Centre out"].map(v=>`<option ${s.gradientDirection===v?"selected":""}>${v}</option>`).join("")}</select></div></div><div class="studio-toggle-row"><label><input type="checkbox" data-studio-check="sortLightDark" ${s.sortLightDark?"checked":""}> Sort yarn colours light to dark</label><label><input type="checkbox" data-studio-check="reverseGradient" ${s.reverseGradient?"checked":""}> Reverse gradient</label></div>${shared}</div>`;
  if(studio.mode==="Random Mix Mode")return `<div class="studio-settings-panel"><div class="form-grid"><div class="field"><label for="studio-setting-totalBlocks">Total rows or blocks</label><input id="studio-setting-totalBlocks" data-studio-setting="totalBlocks" type="number" min="1" value="${s.totalBlocks}"></div><div class="field"><label for="studio-setting-minRepeat">Minimum repeat</label><input id="studio-setting-minRepeat" data-studio-setting="minRepeat" type="number" min="1" value="${s.minRepeat}"></div><div class="field"><label for="studio-setting-maxRepeat">Maximum repeat</label><input id="studio-setting-maxRepeat" data-studio-setting="maxRepeat" type="number" min="1" value="${s.maxRepeat}"></div><div class="field"><label for="studio-setting-randomSeed">Random seed</label><input id="studio-setting-randomSeed" data-studio-setting="randomSeed" value="${escapeHtml(s.randomSeed)}"></div></div><p class="muted-copy">Colour card weights set the random chance. Locked colours keep their card details while the seed regenerates the same mix.</p>${shared}</div>`;
  if(studio.mode==="Colour Pooling Mode")return `<div class="studio-settings-panel"><div class="form-grid"><div class="field"><label for="studio-setting-colourRepeatLength">Colour repeat length</label><input id="studio-setting-colourRepeatLength" data-studio-setting="colourRepeatLength" type="number" min="1" value="${s.colourRepeatLength}"></div><div class="field"><label for="studio-setting-stitchesPerRepeat">Stitches per colour repeat</label><input id="studio-setting-stitchesPerRepeat" data-studio-setting="stitchesPerRepeat" type="number" min="1" value="${s.stitchesPerRepeat}"></div><div class="field"><label for="studio-setting-rowWidth">Row width in stitches</label><input id="studio-setting-rowWidth" data-studio-setting="rowWidth" type="number" min="1" value="${s.rowWidth}"></div><div class="field"><label for="studio-setting-rows">Number of rows</label><input id="studio-setting-rows" data-studio-setting="rows" type="number" min="1" value="${s.rows}"></div><div class="field"><label for="studio-setting-stitchesPerSegment">Stitches per colour segment</label><input id="studio-setting-stitchesPerSegment" data-studio-setting="stitchesPerSegment" type="number" min="1" value="${s.stitchesPerSegment}"></div><div class="field"><label for="studio-setting-targetShift">Target shift per row</label><input id="studio-setting-targetShift" data-studio-setting="targetShift" type="number" value="${s.targetShift}"></div></div><p class="muted-copy">Colour pooling is highly sensitive to tension and yarn repeat length. Swatch before starting the full project.</p>${shared}</div>`;
  return `<div class="studio-settings-panel"><div class="form-grid"><div class="field"><label for="studio-setting-rows">Number of rows</label><input id="studio-setting-rows" data-studio-setting="rows" type="number" min="2" value="${s.rows}"></div><div class="field"><label for="studio-setting-rowWidth">Row width / stitches</label><input id="studio-setting-rowWidth" data-studio-setting="rowWidth" type="number" min="4" value="${s.rowWidth}"></div><div class="field"><label for="studio-setting-rowsPerStripe">Rows per stripe</label><input id="studio-setting-rowsPerStripe" data-studio-setting="rowsPerStripe" type="number" min="1" value="${s.rowsPerStripe}"></div><div class="field"><label for="studio-setting-repeatCount">Repeat count</label><input id="studio-setting-repeatCount" data-studio-setting="repeatCount" type="number" min="1" value="${s.repeatCount}"></div><div class="field"><label for="studio-setting-stripeWidthMode">Stripe width</label><select id="studio-setting-stripeWidthMode" data-studio-setting="stripeWidthMode">${["Equal width","Random weighted width","Custom sequence"].map(v=>`<option ${s.stripeWidthMode===v?"selected":""}>${v}</option>`).join("")}</select></div><div class="field"><label for="studio-setting-customSequence">Stripe sequence</label><input id="studio-setting-customSequence" data-studio-setting="customSequence" value="${escapeHtml(s.customSequence)}" placeholder="1,2,3,2"></div><div class="field"><label for="studio-setting-borderColor">Border colour</label><input id="studio-setting-borderColor" data-studio-setting="borderColor" value="${escapeHtml(s.borderColor)}" placeholder="#f5ead8 or cream"></div></div><div class="studio-toggle-row"><label><input type="checkbox" data-studio-check="mirrorRepeat" ${s.mirrorRepeat?"checked":""}> Mirror repeat</label><label><input type="checkbox" data-studio-check="alternateRepeat" ${s.alternateRepeat?"checked":""}> Alternate repeat</label><label><input type="checkbox" data-studio-check="lockOrder" ${s.lockOrder?"checked":""}> Lock colour order</label></div>${shared}</div>`;
}
...
function renderingStudioContent(p,tool=currentProjectTool){
  ...
  return `<section class="rendering-studio" data-studio-version="${escapeHtml(studio.version)}">
    ...
    <div class="studio-input-grid">
      <div class="studio-panel studio-project-panel"><h3>Project and pattern</h3><div class="studio-form-grid"><div class="field"><label for="studio-project-type">Project type</label><select id="studio-project-type">${studioProjectTypes.map(t=>`<option ${studio.projectType===t?"selected":""}>${t}</option>`).join("")}</select></div><div class="field"><label for="studio-pattern-mode">Preview mode</label><select id="studio-pattern-mode">${studioPatternModes.map(m=>`<option ${studio.mode===m?"selected":""}>${m}</option>`).join("")}</select></div></div>${studioSettingsHtml(studio)}</div>
    </div>
    <section class="studio-panel studio-add-panel"><div class="section-heading compact-row"><div><p class="eyebrow">COLOUR PLANNING</p><h3>Add colour</h3><p class="muted-copy">Add yarn colours by name, hex, stash source, or scan, then manage the colours used in the preview.</p></div></div><div class="studio-add-grid"><div class="field"><label for="studio-new-name">Colour / yarn colour name</label><input id="studio-new-name" value="" placeholder="Sage, oat, dye colourway"></div><div class="field"><label for="studio-new-hex">Hex code</label><input id="studio-new-hex" value="" placeholder="#718c72"></div><div class="field"><label for="studio-new-brand">Source / stash note</label><input id="studio-new-brand" value="" placeholder="Brand or source"></div><div class="field"><label for="studio-new-line">Yarn weight / ball info</label><input id="studio-new-line" value="" placeholder="DK, 100 g, merino..."></div><div class="field"><label for="studio-new-weight">Ball count / weight count</label><input id="studio-new-weight" type="number" min="1" max="10" value="1"></div><div class="field full"><label for="studio-stash-colour">Use colour from Yarn Stash</label><select id="studio-stash-colour" multiple size="4">${stash.length?stash.map(i=>`<option value="${i.id}">${escapeHtml(yarnItemDisplayName(i))}${i.colourNumber?` · ${escapeHtml(i.colourNumber)}`:""}${validHex(yarnItemColor(i))?"":" · needs colour"}</option>`).join(""):`<option disabled>No yarn saved yet</option>`}</select><small>Select one or more saved yarns. If a yarn has no colour yet, capture or choose its colour first.</small></div></div><div class="studio-button-row"><button class="secondary-button" id="studio-add-colour">Add colour</button><button class="mini-button" id="studio-open-scan-yarn">Scan Yarn</button></div><div class="studio-current-colours"><div class="section-heading compact-row"><div><p class="eyebrow">CURRENT COLOURS</p><h3>Added colours</h3></div><span>${studio.colors.length} colour${studio.colors.length===1?"":"s"}</span></div><div class="studio-colour-board">${colourCards}</div></div></section>
...
function projectToolContent(p, tool) {
  ...
  if(tool==="swatch")return card(`<div class="form-grid"><div class="field"><label for="g-pattern-st">Pattern gauge stitches</label><input id="g-pattern-st" type="number" value="${saved.patternSt||20}" placeholder="e.g. 20"></div><div class="field"><label for="g-pattern-row">Pattern gauge rows</label><input id="g-pattern-row" type="number" value="${saved.patternRows||28}" placeholder="e.g. 28"></div><div class="field"><label for="g-user-st">What you got: stitches</label><input id="g-user-st" type="number" value="${saved.userSt||22}" placeholder="e.g. 22"></div><div class="field"><label for="g-user-row">What you got: rows</label><input id="g-user-row" type="number" value="${saved.userRows||30}" placeholder="e.g. 30"></div><div class="field"><label for="g-original-st">Original stitch count</label><input id="g-original-st" type="number" value="${saved.originalSt||100}" placeholder="e.g. 100"></div><div class="field"><label for="g-original-row">Original row count</label><input id="g-original-row" type="number" value="${saved.originalRows||80}" placeholder="e.g. 80"></div></div>`);
  if(tool==="shaping")return card(`<div class="form-grid"><div class="field"><label for="shape-current">How many stitches now?</label><input id="shape-current" type="number" value="${saved.current||84}" placeholder="e.g. 84"></div><div class="field"><label for="shape-desired">How many stitches do you need?</label><input id="shape-desired" type="number" value="${saved.desired||96}" placeholder="e.g. 96"></div><div class="field"><label for="shape-row">Where are you applying this?</label><input id="shape-row" type="number" value="${saved.row||p.row}" placeholder="Current row"></div></div>`, "Plan shaping");
  if(tool==="garment")return card(`<h3>Garment Resizer</h3><p class="muted-copy">For cardigans, sweaters, tops, vests, dresses, and other clothing projects.</p><div class="form-grid"><div class="field"><label for="garment-original-st">Original stitch count</label><input id="garment-original-st" type="number" value="${saved.originalSt||54}"></div><div class="field"><label for="garment-repeat">Original repeat size</label><input id="garment-repeat" type="number" value="${saved.repeat||2}"></div><div class="field"><label for="garment-left">Left section</label><input id="garment-left" type="number" value="${saved.left||6}"></div><div class="field"><label for="garment-centre">Centre / back section</label><input id="garment-centre" type="number" value="${saved.centre||15}"></div><div class="field"><label for="garment-right">Right section</label><input id="garment-right" type="number" value="${saved.right||6}"></div><div class="field"><label for="garment-desired">What stitch count do you want?</label><input id="garment-desired" type="number" value="${saved.desired||40}"></div><div class="field"><label for="garment-symmetry">Keep symmetry?</label><select id="garment-symmetry"><option value="yes" ${saved.symmetry!=="no"?"selected":""}>Yes</option><option value="no" ${saved.symmetry==="no"?"selected":""}>No</option></select></div><div class="field"><label for="garment-multiple">Keep repeat multiple?</label><select id="garment-multiple"><option value="yes" ${saved.multiple!=="no"?"selected":""}>Yes</option><option value="no" ${saved.multiple==="no"?"selected":""}>No</option></select></div></div>`, "Resize garment");
  if(tool==="tool-adjust")return card(`<div class="form-grid"><div class="field"><label for="adj-pattern-tool">Pattern needle/hook size</label><input id="adj-pattern-tool" type="number" step=".25" value="${saved.patternTool||4}" placeholder="e.g. 4 mm"></div><div class="field"><label for="adj-user-tool">Your needle/hook size</label><input id="adj-user-tool" type="number" step=".25" value="${saved.userTool||4.5}" placeholder="e.g. 4.5 mm"></div><div class="field"><label for="adj-pattern-gauge">Pattern gauge</label><input id="adj-pattern-gauge" type="number" value="${saved.patternGauge||20}" placeholder="stitches per 10 cm"></div><div class="field"><label for="adj-user-gauge">Your gauge</label><input id="adj-user-gauge" type="number" value="${saved.userGauge||18}" placeholder="stitches per 10 cm"></div><div class="field"><label for="adj-desired-size">Desired finished size</label><input id="adj-desired-size" type="number" value="${saved.desiredSize||50}" placeholder="cm or inches"></div><div class="field"><label for="adj-current-size">Current swatch size</label><input id="adj-current-size" type="number" value="${saved.currentSize||55}" placeholder="same unit"></div></div>`, "Check size risk");
  if(tool==="yarn-estimator")return card(`<div class="form-grid"><div class="field"><label for="yarn-kind">Project kind</label><select id="yarn-kind">${["Blanket","Sweater","Hat / beanie","Scarf","Socks pair","Amigurumi","Shawl","Gloves pair","Custom dimension"].map(k=>`<option ${saved.kind===k?"selected":""}>${k}</option>`).join("")}</select></div><div class="field"><label for="yarn-size">Size</label><input id="yarn-size" value="${escapeHtml(saved.size||"M")}" placeholder="e.g. M, adult, baby"></div><div class="field"><label for="yarn-weight-tool">Yarn weight</label><select id="yarn-weight-tool">${yarnWeights.map(r=>`<option>${r.name}</option>`).join("")}</select></div><div class="field"><label for="yarn-length">Length per ball/skein (${preferredUnit("yarnLength")})</label><input id="yarn-length" type="number" value="${saved.length||(state.unitSystem==="imperial"?220:200)}" placeholder="${preferredUnit("yarnLength")}"></div><div class="field"><label for="yarn-ball-weight">Weight per ball/skein (${preferredUnit("weight")})</label><input id="yarn-ball-weight" type="number" value="${saved.ballWeight||(state.unitSystem==="imperial"?3.5:100)}" placeholder="${preferredUnit("weight")}"></div><div class="field"><label for="yarn-pattern-amount">Pattern estimated yarn</label><input id="yarn-pattern-amount" type="number" value="${saved.patternAmount||900}" placeholder="same length unit"></div><div class="field"><label for="yarn-mod">User modification %</label><input id="yarn-mod" type="number" value="${saved.mod||0}" placeholder="e.g. -10 or 15"></div><div class="field"><label for="yarn-owned">How many balls do you already have?</label><input id="yarn-owned" type="number" value="${saved.owned||0}"></div></div>`, "Estimate yarn");
  if(tool==="budget")return card(`<div class="form-grid"><div class="field"><label for="budget-yarn-price">Yarn price per ball</label><input id="budget-yarn-price" type="number" step=".01" value="${saved.yarnPrice||12}"></div><div class="field"><label for="budget-balls">Balls needed</label><input id="budget-balls" type="number" value="${saved.balls||6}"></div><div class="field"><label for="budget-supplies">Tools / supplies cost</label><input id="budget-supplies" type="number" step=".01" value="${saved.supplies||20}"></div><div class="field"><label for="budget-items">Finished items made</label><input id="budget-items" type="number" value="${saved.items||1}"></div><div class="field"><label for="budget-leftover">Leftover yarn balls</label><input id="budget-leftover" type="number" step=".1" value="${saved.leftover||.5}"></div></div>`, "Calculate budget");
  if(tool==="cast-on")return card(`<div class="form-grid"><div class="field"><label for="simple-width">Finished width</label><input id="simple-width" type="number" value="40"></div><div class="field"><label for="simple-gauge">${preferredUnit("gauge")}</label><input id="simple-gauge" type="number" value="22"></div><div class="field"><label for="simple-multiple">Stitch multiple</label><input id="simple-multiple" type="number" value="2"></div><div class="field"><label for="simple-edge">Edge stitches</label><input id="simple-edge" type="number" value="2"></div></div>`, "Calculate cast on");
  if(tool==="sock")return card(`<div class="form-grid"><div class="field"><label for="sock-circ">Foot circumference</label><input id="sock-circ" type="number" value="22"></div><div class="field"><label for="sock-gauge">${preferredUnit("gauge")}</label><input id="sock-gauge" type="number" value="30"></div><div class="field"><label for="sock-ease">Ease %</label><input id="sock-ease" type="number" value="10"></div><div class="field"><label for="sock-length">Foot length</label><input id="sock-length" type="number" value="24"></div></div>`, "Plan sock");
  if(tool==="circle")return card(`<div class="form-grid"><div class="field"><label for="circle-start">Starting stitches</label><input id="circle-start" type="number" value="6"></div><div class="field"><label for="circle-rounds">Rounds</label><input id="circle-rounds" type="number" value="8"></div><div class="field"><label for="circle-stitch">Stitch type</label><select id="circle-stitch"><option>sc</option><option>hdc</option><option>dc</option></select></div></div>`, "Plan circle");
  if(tool==="amigurumi")return card(`<div class="form-grid"><div class="field"><label for="ami-shape">Shape</label><select id="ami-shape"><option>round</option><option>oval</option><option>cone</option><option>cylinder</option></select></div><div class="field"><label for="ami-base">Base stitches</label><input id="ami-base" type="number" value="${saved.baseStitches||6}"></div><div class="field"><label for="ami-rounds">Increase rounds</label><input id="ami-rounds" type="number" value="${saved.increaseRounds||6}"></div><div class="field"><label for="ami-straight">Straight rounds</label><input id="ami-straight" type="number" value="${saved.straightRounds||6}"></div><div class="field"><label for="ami-chain">Oval chain length</label><input id="ami-chain" type="number" value="${saved.chainLength||8}"></div></div>`, "Guide shape");
  if(tool==="granny")return card(`<div class="form-grid"><div class="field"><label for="granny-target">Target square size</label><input id="granny-target" type="number" value="${saved.targetSize||12}"></div><div class="field"><label for="granny-size">Size per round</label><input id="granny-size" type="number" value="${saved.sizePerRound||2.5}"></div><div class="field"><label for="granny-join">Joining allowance</label><input id="granny-join" type="number" value="${saved.joiningAllowance||0.5}"></div><div class="field"><label for="granny-width">Project width</label><input id="granny-width" type="number" value="${saved.blanketWidth||120}"></div><div class="field"><label for="granny-length">Project length</label><input id="granny-length" type="number" value="${saved.blanketLength||120}"></div></div>`, "Plan squares");
  if(tool==="c2c")return card(`<div class="form-grid"><div class="field"><label for="c2c-width">Width</label><input id="c2c-width" type="number" value="120"></div><div class="field"><label for="c2c-height">Height</label><input id="c2c-height" type="number" value="150"></div><div class="field"><label for="c2c-block">Block size</label><input id="c2c-block" type="number" value="2.5"></div><div class="field"><label for="c2c-yarn">Yarn per block</label><input id="c2c-yarn" type="number" value="1.1"></div></div>`, "Plan C2C");
  if(tool==="hat")return card(`<div class="form-grid"><div class="field"><label for="hat-circ">Head circumference</label><input id="hat-circ" type="number" value="56"></div><div class="field"><label for="hat-ease">Fit ease %</label><input id="hat-ease" type="number" value="8"></div><div class="field"><label for="hat-gauge">${preferredUnit("gauge")}</label><input id="hat-gauge" type="number" value="20"></div></div>`, "Calculate hat");
  if(tool==="sleeve")return card(`<div class="form-grid"><div class="field"><label for="sleeve-start">Start stitches</label><input id="sleeve-start" type="number" value="64"></div><div class="field"><label for="sleeve-end">End stitches</label><input id="sleeve-end" type="number" value="42"></div><div class="field"><label for="sleeve-rows">Rows available</label><input id="sleeve-rows" type="number" value="90"></div></div>`, "Plan sleeve");
  if(tool==="raglan")return card(`<div class="form-grid"><div class="field"><label for="raglan-neck">Neck stitches</label><input id="raglan-neck" type="number" value="88"></div><div class="field"><label for="raglan-target">Target body + sleeves stitches</label><input id="raglan-target" type="number" value="260"></div><div class="field"><label for="raglan-inc">Increase stitches per increase round</label><input id="raglan-inc" type="number" value="8"></div></div>`, "Plan raglan");
  if(tool==="blanket")return card(`<div class="form-grid"><div class="field"><label for="blanket-width">Width</label><input id="blanket-width" type="number" value="100"></div><div class="field"><label for="blanket-height">Height</label><input id="blanket-height" type="number" value="140"></div><div class="field"><label for="blanket-st">${preferredUnit("gauge")}</label><input id="blanket-st" type="number" value="16"></div><div class="field"><label for="blanket-row">Rows per ${state.unitSystem==="imperial"?"4 in":"10 cm"}</label><input id="blanket-row" type="number" value="20"></div><div class="field"><label for="blanket-yarn">Yarn per 10x10 area</label><input id="blanket-yarn" type="number" value="12"></div></div>`, "Plan blanket");
  if(tool==="grid")return card(`<div class="form-grid"><div class="field"><label for="grid-width">Width cells</label><input id="grid-width" type="number" value="${saved.width||40}"></div><div class="field"><label for="grid-height">Height cells</label><input id="grid-height" type="number" value="${saved.height||60}"></div><div class="field"><label for="grid-craft">Craft type</label><select id="grid-craft"><option>knitting</option><option>crochet</option></select></div><div class="field"><label for="grid-symbols">Symbol set</label><select id="grid-symbols"><option>basic</option><option>lace</option><option>colourwork</option></select></div><div class="field"><label for="grid-colors">Colours</label><input id="grid-colors" type="number" value="${saved.colors||2}"></div></div>`, "Generate grid");
  if(tool==="stripe")return card(`<div class="form-grid"><div class="field"><label for="stripe-direction">Direction</label><select id="stripe-direction"><option>horizontal stripes</option><option>vertical stripes</option></select></div><div class="field"><label for="stripe-total">Total rows/stitches</label><input id="stripe-total" type="number" value="120"></div><div class="field"><label for="stripe-colors">Colours</label><input id="stripe-colors" type="number" value="4"></div></div>`, "Plan stripes");
  if(tool==="pooling")return card(`<div class="form-grid"><div class="field"><label for="pool-repeat">Colour repeat length</label><input id="pool-repeat" type="number" value="${saved.colorRepeatLength||60}"></div><div class="field"><label for="pool-stitches">Stitches per repeat</label><input id="pool-stitches" type="number" value="${saved.stitchesPerRepeat||24}"></div><div class="field"><label for="pool-row-width">Row width</label><input id="pool-row-width" type="number" value="${saved.rowWidth||96}"></div><div class="field full"><p class="muted-copy">Colour matching recommendations will be expanded later.</p></div></div>`, "Plan pooling");
  if(tool==="blocking")return card(`<div class="form-grid"><div class="field"><label for="block-width">Before width</label><input id="block-width" type="number" value="${saved.beforeWidth||saved.w||48}"></div><div class="field"><label for="block-height">Before length</label><input id="block-height" type="number" value="${saved.beforeLength||saved.h||60}"></div><div class="field"><label for="block-after-width">After width</label><input id="block-after-width" type="number" value="${saved.afterWidth||""}" placeholder="optional"></div><div class="field"><label for="block-after-height">After length</label><input id="block-after-height" type="number" value="${saved.afterLength||""}" placeholder="optional"></div><div class="field"><label for="block-growth">Expected growth %</label><input id="block-growth" type="number" value="${saved.growthPercent??saved.growth??8}" placeholder="used if after size is blank"></div></div>`, "Estimate blocking");
  ...
  if(tool==="repeat")return card(`<div class="form-grid"><div class="field"><label for="repeat-total">How many stitches/rows do you have?</label><input id="repeat-total" type="number" value="${saved.total||120}"></div><div class="field"><label for="repeat-size">How many are in one repeat?</label><input id="repeat-size" type="number" value="${saved.repeat||8}"></div><div class="field"><label for="repeat-edge">Edge stitches to keep aside</label><input id="repeat-edge" type="number" value="${saved.edge||0}"></div></div>`, "Calculate repeats");
  if(tool==="row-helper")return card(`<div class="form-grid"><div class="field"><label for="row-current">Current row/round</label><input id="row-current" type="number" value="${saved.current||p.row}"></div><div class="field"><label for="row-total">Planned rows/rounds</label><input id="row-total" type="number" value="${saved.total||p.totalRows||100}"></div><div class="field"><label for="row-repeat">Repeat every X rows</label><input id="row-repeat" type="number" value="${saved.repeat||6}"></div></div>`, "Plan rows");
  if(tool==="substitution")return card(`<div class="form-grid"><div class="field"><label for="sub-pattern-length">Pattern yarn length per ball</label><input id="sub-pattern-length" type="number" value="${saved.patternLength||220}"></div><div class="field"><label for="sub-new-length">Substitute yarn length per ball</label><input id="sub-new-length" type="number" value="${saved.newLength||180}"></div><div class="field"><label for="sub-pattern-balls">Pattern says how many balls?</label><input id="sub-pattern-balls" type="number" value="${saved.patternBalls||6}"></div><div class="field"><label for="sub-gauge-diff">Gauge difference %</label><input id="sub-gauge-diff" type="number" value="${saved.gaugeDiff||0}"></div></div>`, "Compare yarns");
  if(tool==="yarn-leftover")return card(`<div class="form-grid"><div class="field"><label for="leftover-weight">Leftover weight (${preferredUnit("weight")})</label><input id="leftover-weight" type="number" value="${saved.leftoverWeight||(state.unitSystem==="imperial"?1.75:50)}"></div><div class="field"><label for="leftover-skein-weight">Original skein weight (${preferredUnit("weight")})</label><input id="leftover-skein-weight" type="number" value="${saved.originalSkeinWeight||(state.unitSystem==="imperial"?3.5:100)}"></div><div class="field"><label for="leftover-skein-length">Original skein length (${preferredUnit("yarnLength")})</label><input id="leftover-skein-length" type="number" value="${saved.originalSkeinLength||(state.unitSystem==="imperial"?220:200)}"></div></div>`, "Estimate leftover");
  if(tool==="yarn-weight")return card(`<div class="form-grid"><div class="field"><label for="tw-grams">Ball weight (${preferredUnit("weight")})</label><input id="tw-grams" type="number" value="${saved.grams||(state.unitSystem==="imperial"?3.5:100)}"></div><div class="field"><label for="tw-yards">Length (${preferredUnit("yarnLength")})</label><input id="tw-yards" type="number" value="${saved.yards||(state.unitSystem==="imperial"?220:200)}"></div><div class="field"><label for="tw-wpi">Wraps per inch</label><input id="tw-wpi" type="number" value="${saved.wpi||""}" placeholder="optional"></div></div>`, "Find yarn category");
  return card(`<div class="form-grid"><div class="field"><label for="unit-amount">Input value</label><input id="unit-amount" type="number" value="${saved.amount||100}"></div><div class="field"><label for="unit-from">From unit</label><select id="unit-from">${["cm","mm","m","in","yd","g","oz"].map(u=>`<option ${((saved.from||preferredUnit("length"))===u)?"selected":""}>${u}</option>`).join("")}</select></div><div class="field"><label for="unit-to">To unit</label><select id="unit-to">${["cm","mm","m","in","yd","g","oz"].map(u=>`<option ${((saved.to||(state.unitSystem==="imperial"?"yd":"m"))===u)?"selected":""}>${u}</option>`).join("")}</select></div></div>`, "Convert");
}
...
function exportStudioImage(studio){
  ...
  canvas.width=w;canvas.height=h;ctx.fillStyle="#fff9f0";ctx.fillRect(0,0,w,h);ctx.fillStyle="#5a4632";ctx.font="700 34px 'DM Sans'";ctx.fillText(`${studio.projectType} - ${studio.mode}`,48,70);ctx.font="18px 'DM Sans'";ctx.fillText(studioPaletteText(studio),48,108);
...
function openYarnScanReviewModal(draft={}){
  ...
  openModal(`<p class="eyebrow">REVIEW SCANNED YARN</p><h2>Save to Yarn Stash</h2><p class="muted-copy">Correct anything that looks off before saving. Barcode does not guarantee exact shade or dye lot.</p>
    <div class="form-grid">
      <div class="field"><label for="scan-brand">Brand</label><input id="scan-brand" value="${escapeHtml(draft.brand||"")}"></div><div class="field"><label for="scan-yarn-name">Yarn name</label><input id="scan-yarn-name" value="${escapeHtml(draft.yarnName||draft.name||"")}"></div>
      <div class="field"><label for="scan-barcode-value">Barcode</label><input id="scan-barcode-value" value="${escapeHtml(draft.barcode||"")}"></div><div class="field"><label for="scan-colour-name">Colour name</label><input id="scan-colour-name" value="${escapeHtml(draft.colourName||"")}"></div>
      <div class="field"><label for="scan-colour-number">Colour number</label><input id="scan-colour-number" value="${escapeHtml(draft.colourNumber||"")}"></div><div class="field"><label for="scan-dye-lot">Dye lot</label><input id="scan-dye-lot" value="${escapeHtml(draft.dyeLot||"")}"></div>
      <div class="field"><label for="scan-colour-hex">Colour HEX</label><input id="scan-colour-hex" type="color" value="${validHex(draft.colourHex)?draft.colourHex:"#718c72"}"></div><div class="field"><label for="scan-colour-source">Colour source</label><select id="scan-colour-source">${["barcode","label-ocr","photo-scan","manual"].map(v=>`<option value="${v}" ${(draft.colourSource||"manual")===v?"selected":""}>${v}</option>`).join("")}</select></div>
      <div class="field"><label for="scan-weight">Weight</label><input id="scan-weight" value="${escapeHtml(draft.weight||"")}"></div><div class="field"><label for="scan-fibre">Fibre content</label><input id="scan-fibre" value="${escapeHtml(draft.fibreContent||"")}"></div>
      <div class="field"><label for="scan-grams">Grams</label><input id="scan-grams" type="number" value="${draft.grams||""}"></div><div class="field"><label for="scan-metres">Metres</label><input id="scan-metres" type="number" value="${draft.metres||""}"></div>
      <div class="field"><label for="scan-yards">Yards</label><input id="scan-yards" type="number" value="${draft.yards||""}"></div><div class="field"><label for="scan-quantity">Quantity</label><input id="scan-quantity" type="number" min="0" value="${draft.quantity||1}"></div>
      <div class="field"><label for="scan-hook">Hook suggestion</label><input id="scan-hook" value="${escapeHtml(draft.hookSize||"")}"></div><div class="field"><label for="scan-needle">Needle suggestion</label><input id="scan-needle" value="${escapeHtml(draft.needleSize||"")}"></div>
      <div class="field full"><label for="scan-notes">Notes</label><textarea id="scan-notes" rows="4">${escapeHtml(draft.notes||"")}</textarea></div>
...
function bindProjectDetail() {
  ...
  document.getElementById("project-notes")?.addEventListener("input", e => { p.notes=e.target.value;p.updatedAt=new Date().toISOString();saveStateSoon(500); });
  document.getElementById("project-notes")?.addEventListener("blur",flushPendingSave);
  document.getElementById("pdf-reference")?.addEventListener("input", e => { p.pdfReference=e.target.value;p.updatedAt=new Date().toISOString();saveStateSoon(500); });
  document.getElementById("pdf-reference")?.addEventListener("blur",flushPendingSave);
...
function addCurrentToolResultToBuyList(){
  ...
  openModal(`<p class="eyebrow">BUY LIST</p><h2>Add this to Buy List?</h2><p class="muted-copy">Nothing is added until you confirm. The project Buy List total updates immediately after saving.</p><div class="form-grid"><div class="field full"><label for="buy-from-tool-name">Item name</label><input id="buy-from-tool-name" value="${escapeHtml(isYarn?"Yarn for "+payload.toolName:"Supply for "+payload.toolName)}"></div><div class="field"><label for="buy-from-tool-category">Category</label><select id="buy-from-tool-category"><option ${isYarn?"selected":""}>Yarn</option><option>Tools</option><option>DIY kits</option><option ${!isYarn?"selected":""}>Other</option></select></div><div class="field"><label for="buy-from-tool-qty">Quantity</label><input id="buy-from-tool-qty" type="number" step="0.1" value="1"></div><div class="field"><label for="buy-from-tool-price">Estimated price</label><input id="buy-from-tool-price" type="number" step="0.01" value="0"></div><div class="field full"><label for="buy-from-tool-notes">Notes</label><textarea id="buy-from-tool-notes" rows="4">${escapeHtml(payload.outputs.result)}</textarea></div></div><div class="modal-actions"><button class="secondary-button" onclick="closeModal()">Cancel</button><button class="primary-button" id="confirm-buy-from-tool">Add to Buy List</button></div>`);
...
function openChartRowModal(rowId=null){
  ...
  openModal(`<p class="eyebrow">CHECKED ROW</p><h2>${row?"Edit":"Add"} written row</h2><div class="form-grid"><div class="field"><label for="analysis-row-number">Row number</label><input id="analysis-row-number" type="number" min="1" value="${row?.number||p.row||1}"></div><div class="field"><label for="analysis-row-side">Side</label><select id="analysis-row-side">${["Right side","Wrong side","Worked in the round","Side uncertain"].map(v=>`<option ${row?.side===v?"selected":""}>${v}</option>`).join("")}</select></div><div class="field full"><label for="analysis-row-sequence">Stitch sequence</label><textarea id="analysis-row-sequence" rows="4" placeholder="Use “uncertain” for every unreadable cell">${escapeHtml(row?.sequence||"")}</textarea></div><div class="field"><label for="analysis-stitch-count">Resulting stitch count</label><input id="analysis-stitch-count" type="number" min="0" value="${row?.stitchCount||""}" placeholder="Leave blank if uncertain"></div><div class="field"><label for="analysis-row-status">Confidence</label><select id="analysis-row-status"><option value="uncertain" ${row?.status!=="checked"?"selected":""}>Uncertain</option><option value="checked" ${row?.status==="checked"?"selected":""}>Checked by me</option></select></div><div class="field full"><label for="analysis-shaping">Increase / decrease notes</label><input id="analysis-shaping" value="${escapeHtml(row?.shaping||"")}" placeholder="e.g. 2 increases, final count 24"></div><div class="field full"><label for="analysis-correction-destination">Correction destination</label><select id="analysis-correction-destination"><option value="flow">Save only as Flow Mode correction</option><option value="database">Save to Symbol Database</option></select></div></div><label class="check-row"><input id="analysis-learn-toggle" type="checkbox" checked><span>Learn from this symbol</span></label><p class="privacy-note">Yarncha will remember this symbol for future chart reading. Yarncha remembers your corrections on this device.</p><div class="modal-actions"><button class="secondary-button" onclick="closeModal()">Cancel</button><button class="primary-button" id="save-analysis-row">Save row</button></div>`);
...
async function extractPdfText(file,selectedPages=[]){
  try{
    if(file.size>25*1024*1024)throw new Error("PDF exceeds the 25 MB processing limit.");
    const {pdfjsLib:pdfjs}=await getDocumentTools();
    const pdf=await pdfjs.getDocument({data:new Uint8Array(await file.arrayBuffer()),isEvalSupported:false,disableFontFace:false}).promise;
    const pages=[];
...
async function getDocumentTools(){
  ...
  if(!window.YarnchaDocumentToolsLoader?.load)throw new Error("Document tools loader is unavailable.");
  return window.YarnchaDocumentToolsLoader.load();
}
    ...
    <div class="ocr-review-layout">
      ...
      <div class="ocr-review-panel">
        ...
        <div class="form-grid compact-form">
          <div class="field"><label for="ocr-selected-pages">PDF pages</label><input id="ocr-selected-pages" value="${escapeHtml(pageList)}" placeholder="1, 2, 3"></div>
          <div class="field"><label for="ocr-rotation">Rotate image</label><select id="ocr-rotation">${[0,90,180,270].map(v=>`<option value="${v}" ${source.scanSettings.rotation===v?"selected":""}>${v}°</option>`).join("")}</select></div>
          <div class="field"><label for="ocr-zoom">Zoom preview</label><input id="ocr-zoom" type="number" min=".5" max="3" step=".1" value="${source.scanSettings.zoom}"></div>
          <div class="field"><label for="ocr-crop-x">Crop left %</label><input id="ocr-crop-x" type="number" min="0" max="100" value="${source.scanSettings.crop.x}"></div>
          <div class="field"><label for="ocr-crop-y">Crop top %</label><input id="ocr-crop-y" type="number" min="0" max="100" value="${source.scanSettings.crop.y}"></div>
          <div class="field"><label for="ocr-crop-width">Crop width %</label><input id="ocr-crop-width" type="number" min="1" max="100" value="${source.scanSettings.crop.width}"></div>
          <div class="field"><label for="ocr-crop-height">Crop height %</label><input id="ocr-crop-height" type="number" min="1" max="100" value="${source.scanSettings.crop.height}"></div>
...
function yarnMaterial({id,name,category,bestFor=[],notIdealFor=[],shortExplanation,behaviour,warning,careLevel="moderate",beginnerFriendly=true,warmth="medium",softness="medium",elasticity="medium",drape="medium",durability="medium",stitchDefinition="medium",careTips="Check the yarn label and wash a swatch first.",substitutionNotes="Match gauge, fibre behaviour, and metres per 100 g—not weight name alone.",tags=[],relatedTools=["Yarn Substitution Helper","Gauge / Swatch Adapter","Yarn Estimator"]}){
  return {id:`ym-${id}`,name,category,origin:category,beginnerFriendly,warmth,softness,elasticity,drape,durability,stitchDefinition,careLevel,bestFor,notIdealFor,shortExplanation,behaviour,commonUses:bestFor.join(", "),commonProblems:warning,mainWarning:warning,careTips,substitutionNotes,tags:[category,...tags,...bestFor],relatedTools,season:tags.includes("summer")?"Warm weather":tags.includes("winter")?"Cool weather":"All season",textures:[softness==="high"?"Soft":"Textured",drape==="high"?"Drapey":elasticity==="high"?"Springy":"Structured"],features:shortExplanation,builtIn:true};
}
function defaultYarnMaterials(){
  const m=yarnMaterial;
  const fibres=[
    m({id:"wool",name:"Wool",category:"Animal fibres",warmth:"high",elasticity:"high",durability:"high",stitchDefinition:"high",bestFor:["Sweaters","Cardigans","Hats","Scarves","Mittens","Socks","Colourwork","Cables"],notIdealFor:["Sensitive skin","Hot weather","Unlabelled machine washing"],shortExplanation:"Warm, breathable and springy, with excellent memory for garments, cables and colourwork.",behaviour:"Recovers shape better than most plant fibres and can felt unless treated.",warning:"Breed and processing affect itch; non-superwash wool can felt.",tags:["garment","sock","winter"]}),
    m({id:"merino",name:"Merino wool",category:"Animal fibres",warmth:"high",softness:"high",elasticity:"high",bestFor:["Sweaters","Baby garments","Hats","Scarves","Shawls"],notIdealFor:["High-friction bags","Rough-use items"],shortExplanation:"A softer wool that is warm, elastic and comfortable next to skin.",behaviour:"Fine fibres feel gentle but may pill faster; superwash versions may grow.",warning:"Soft merino can pill, and superwash merino may stretch after blocking.",tags:["luxury","baby","garment","winter","sensitive skin"]}),
    m({id:"superwash-wool",name:"Superwash wool",category:"Animal fibres",warmth:"high",softness:"high",elasticity:"medium",careLevel:"easy",bestFor:["Socks","Baby items","Gifts","Everyday garments"],notIdealFor:["Projects relying on felting","Unsupported heavy garments"],shortExplanation:"Treated wool that is less likely to felt and is often machine washable.",behaviour:"Smoother than untreated wool, but can lose bounce or grow when wet.",warning:"Machine washable does not automatically mean dryer-safe.",careTips:"Follow the label exactly and measure a washed swatch.",tags:["easy care","baby","sock","garment"]}),
    m({id:"alpaca",name:"Alpaca",category:"Animal fibres",warmth:"high",softness:"high",elasticity:"low",drape:"high",bestFor:["Shawls","Scarves","Cowls","Loose cardigans"],notIdealFor:["Heavy fitted sweaters","Springy ribbing","Unblended socks"],shortExplanation:"Very warm, soft and drapey, with less memory than wool.",behaviour:"Can sag or grow in fitted garments and may shed depending on spin.",warning:"Pure alpaca has little recovery, so structured garments may lengthen.",tags:["luxury","garment","winter"]}),
    m({id:"mohair",name:"Mohair",category:"Luxury fibres",warmth:"high",softness:"high",elasticity:"medium",drape:"high",beginnerFriendly:false,careLevel:"delicate",bestFor:["Sweaters","Cardigans","Shawls","Lace","Holding double"],notIdealFor:["Frequent frogging","Amigurumi","Clear stitch definition","Sensitive skin"],shortExplanation:"Light, warm Angora-goat fibre with a fluffy halo, often held with another yarn.",behaviour:"Fibres cling together, blur detail and can shed.",warning:"Mohair is difficult to rip back cleanly.",tags:["luxury","winter","sensitive skin"]}),
    m({id:"cashmere",name:"Cashmere",category:"Luxury fibres",warmth:"high",softness:"high",elasticity:"medium",careLevel:"delicate",beginnerFriendly:false,bestFor:["Scarves","Cowls","Sweaters","Skin-touch garments"],notIdealFor:["Bags","Heavy wear","Practice projects"],shortExplanation:"Exceptionally soft, light and warm luxury fibre, usually blended.",behaviour:"Loosely spun cashmere may pill and needs gentle handling.",warning:"Expensive, delicate and prone to pilling in friction areas.",tags:["luxury","winter","sensitive skin"]}),
    m({id:"angora",name:"Angora",category:"Luxury fibres",warmth:"high",softness:"high",careLevel:"delicate",beginnerFriendly:false,bestFor:["Luxury accessories","Soft trims","Warm scarves"],notIdealFor:["Hard-wearing items","Fluff-sensitive users","Baby items"],shortExplanation:"Extremely warm, light rabbit fibre with a strong halo.",behaviour:"Often blended for strength; can shed heavily.",warning:"Shedding makes it unsuitable for some babies and sensitive users.",tags:["luxury","winter","sensitive skin"]}),
    m({id:"silk",name:"Silk",category:"Luxury fibres",warmth:"low",softness:"high",elasticity:"low",drape:"high",durability:"high",careLevel:"delicate",bestFor:["Shawls","Lace","Summer tops","Luxury garments"],notIdealFor:["Springy ribbing","Structured garments","Rough washing"],shortExplanation:"Smooth, shiny and strong, adding cool drape and flow.",behaviour:"Has little bounce and can be slippery on tools.",warning:"Beautiful drape comes with low elasticity.",tags:["luxury","summer","garment"]}),
    m({id:"cotton",name:"Cotton",category:"Plant fibres",warmth:"low",softness:"medium",elasticity:"low",drape:"high",durability:"high",stitchDefinition:"high",bestFor:["Summer tops","Bags","Amigurumi","Dishcloths","Baby items","Home items"],notIdealFor:["Stretchy ribbing","Light winter garments","Large unsupported dresses"],shortExplanation:"Cool, smooth and strong with crisp stitches, but very little stretch.",behaviour:"Strong when wet and can become heavy or grow downward.",warning:"Large cotton garments can sag under their own weight.",tags:["summer","bag","amigurumi","baby","garment"]}),
    m({id:"mercerized-cotton",name:"Mercerized cotton",category:"Plant fibres",warmth:"low",elasticity:"low",drape:"medium",durability:"high",stitchDefinition:"high",bestFor:["Crochet lace","Amigurumi","Bags","Summer tops","Decor"],notIdealFor:["Cozy winter garments"],shortExplanation:"Polished cotton treated for shine, strength and very clear stitch definition.",behaviour:"Smooth and crisp with less fuzz, but may feel less cozy.",warning:"Its polished hand can feel firmer than unmercerized cotton.",tags:["summer","bag","amigurumi"]}),
    m({id:"linen",name:"Linen / flax",category:"Plant fibres",warmth:"low",elasticity:"low",drape:"high",durability:"high",bestFor:["Summer tops","Dresses","Light cardigans","Market bags","Shawls"],notIdealFor:["Ribbing","Stretch-dependent projects","Cozy winter items"],shortExplanation:"Strong, breathable flax that starts crisp and softens beautifully with use.",behaviour:"Wrinkles, drapes and has almost no bounce.",warning:"It may feel stiff at first and can be tiring on the hands.",tags:["summer","bag","garment"]}),
    m({id:"bamboo",name:"Bamboo / viscose from bamboo",category:"Plant fibres",warmth:"low",softness:"high",elasticity:"low",drape:"high",bestFor:["Summer garments","Shawls","Scarves","Drapey tops","Baby items"],notIdealFor:["Structured garments","Ribbing","Heavy unsupported pieces"],shortExplanation:"Smooth, cool and often shiny regenerated cellulose with fluid drape.",behaviour:"Can split, stretch and grow after washing or wearing.",warning:"Bamboo gives drape, not bounce or structure.",tags:["summer","baby","garment"]}),
    m({id:"hemp",name:"Hemp",category:"Plant fibres",warmth:"low",elasticity:"low",drape:"medium",durability:"high",bestFor:["Bags","Summer tops","Home items","Durable accessories"],notIdealFor:["Stretch garments","Fluffy projects"],shortExplanation:"Sturdy, breathable fibre that softens with use.",behaviour:"Low stretch makes firm fabric and durable edges.",warning:"Can feel rough or hard on hands until softened.",tags:["summer","bag"]}),
    m({id:"tencel",name:"Tencel / Lyocell",category:"Plant fibres",warmth:"low",softness:"high",elasticity:"low",drape:"high",bestFor:["Summer garments","Shawls","Drapey tops","Soft accessories"],notIdealFor:["Structured ribbing"],shortExplanation:"Cool, smooth wood-pulp fibre that adds shine and flow.",behaviour:"Low elasticity and a slippery surface produce drape rather than recovery.",warning:"It gives drape, not bounce.",tags:["summer","garment"]}),
    m({id:"acrylic",name:"Acrylic",category:"Synthetic fibres",careLevel:"easy",durability:"medium",bestFor:["Blankets","Beginner projects","Amigurumi","Gifts","Kids items"],notIdealFor:["Breathable summer garments","High heat","Luxury drape"],shortExplanation:"Affordable, lightweight, colourful and widely available in easy-care options.",behaviour:"Quality varies; it may pill, squeak or retain less moisture than natural fibre.",warning:"Acrylic can melt under high heat—never press with a hot iron.",tags:["easy care","beginner friendly","baby","amigurumi","winter"]}),
    m({id:"premium-acrylic",name:"Premium acrylic / anti-pilling acrylic",category:"Synthetic fibres",softness:"high",careLevel:"easy",bestFor:["Washable garments","Blankets","Baby items","Gifts"],notIdealFor:["Very breathable summer wear"],shortExplanation:"Softer acrylic engineered for comfort and reduced pilling.",behaviour:"Easy care and practical, though less breathable than wool or cotton.",warning:"Anti-pilling does not mean pill-proof.",tags:["easy care","baby","garment","beginner friendly"]}),
    m({id:"nylon",name:"Nylon / polyamide",category:"Synthetic fibres",elasticity:"high",durability:"high",bestFor:["Socks","Reinforced yarn","Durable blends"],notIdealFor:["Main comfort fibre by itself"],shortExplanation:"Strong support fibre that adds abrasion resistance and recovery.",behaviour:"Usually appears as a smaller percentage in sock and hard-wearing blends.",warning:"Best used as reinforcement rather than the main next-to-skin fibre.",tags:["sock","easy care"]}),
    m({id:"polyester",name:"Polyester",category:"Synthetic fibres",careLevel:"easy",durability:"high",bestFor:["Chenille yarn","Plushies","Blankets","Durable blends"],notIdealFor:["Breathable garments","High heat"],shortExplanation:"Strong, shape-retaining synthetic used in many novelty and plush yarns.",behaviour:"Can be slippery, warm and less breathable.",warning:"Keep away from high heat.",tags:["easy care","amigurumi","baby"]}),
    m({id:"microfiber",name:"Microfiber",category:"Synthetic fibres",softness:"high",careLevel:"easy",bestFor:["Baby blankets","Plush projects","Soft scarves"],notIdealFor:["Structure-heavy projects"],shortExplanation:"Very fine synthetic fibres with a smooth, soft hand.",behaviour:"Comfortable and washable but can lack structure.",warning:"Test dense patterns because the fabric may feel limp.",tags:["easy care","baby"]}),
    m({id:"elastic-blend",name:"Elastic / spandex blend",category:"Blends",elasticity:"high",durability:"high",bestFor:["Socks","Fitted accessories","Stretch garments","Cuffs"],notIdealFor:["Loose drapey fabric"],shortExplanation:"A small elastic percentage helps fabric stretch and return to shape.",behaviour:"Changes recovery and sometimes care requirements.",warning:"Too much elastic can change fabric feel and washing needs.",tags:["sock","garment"]}),
    m({id:"yak",name:"Yak",category:"Luxury fibres",warmth:"high",softness:"high",drape:"high",careLevel:"delicate",bestFor:["Sweaters","Scarves","Cowls","Luxury accessories"],shortExplanation:"Soft, warm luxury fibre often blended with wool or silk.",behaviour:"Adds warmth, softness and elegant drape.",warning:"Treat gently and swatch the blend for recovery.",tags:["luxury","winter"]}),
    m({id:"camel",name:"Camel",category:"Luxury fibres",warmth:"high",softness:"high",careLevel:"delicate",bestFor:["Scarves","Shawls","Sweaters","Winter accessories"],shortExplanation:"Warm, lightweight fibre often used in natural tan blends.",behaviour:"Usually blended to balance softness and structure.",warning:"Follow delicate-care instructions.",tags:["luxury","winter"]}),
    m({id:"qiviut",name:"Qiviut",category:"Luxury fibres",warmth:"high",softness:"high",careLevel:"delicate",beginnerFriendly:false,bestFor:["Luxury scarves","Cowls","Small accessories"],notIdealFor:["Large beginner projects","Rough use"],shortExplanation:"Rare musk-ox undercoat that is extraordinarily warm, soft and expensive.",behaviour:"Best reserved for light, protected accessories.",warning:"Delicate and costly; avoid high-friction projects.",tags:["luxury","winter"]}),
    m({id:"possum-blend",name:"Possum blend",category:"Luxury fibres",warmth:"high",softness:"high",bestFor:["Hats","Gloves","Scarves","Warm garments"],shortExplanation:"Warm, light possum fibre commonly balanced with merino in AU/NZ yarns.",behaviour:"The blend adds loft while merino supplies recovery.",warning:"Check blend-specific washing instructions.",tags:["luxury","winter","garment"]})
  ];
  const blends=[
    ["wool-nylon","Wool nylon blend",["Socks","Gloves","Hard-wearing accessories"],"Wool supplies warmth and bounce; nylon reinforces high-friction areas.","Choose a purpose-made sock blend for best durability.",["sock","winter"]],
    ["wool-acrylic","Wool acrylic blend",["Sweaters","Blankets","Beginner garments","Gifts"],"Warm wool balanced with affordable, easier-care acrylic.","Check the wool percentage and label care.",["garment","easy care","beginner friendly"]],
    ["cotton-acrylic","Cotton acrylic blend",["Baby blankets","Summer tops","Light crochet","Gifts"],"Often lighter and more forgiving than pure cotton.","Still test garment growth and breathability.",["summer","baby","easy care"]],
    ["cotton-bamboo","Cotton bamboo blend",["Summer tops","Shawls","Light cardigans"],"Cool, soft and fluid for warm-weather fabric.","Both fibres have low recovery, so large garments may sag.",["summer","garment"]],
    ["wool-silk","Wool silk blend",["Shawls","Sweaters","Lace","Luxury garments"],"Wool adds bounce while silk adds shine, strength and drape.","Use a washed swatch to balance elasticity and flow.",["luxury","garment"]],
    ["alpaca-wool","Alpaca wool blend",["Sweaters","Cardigans","Hats","Scarves"],"Alpaca adds warmth and drape; wool restores memory.","A high alpaca percentage may still grow.",["garment","winter"]],
    ["mohair-silk","Mohair silk blend",["Fluffy sweaters","Cardigans","Shawls","Holding double"],"A fine silk core carries a light mohair halo.","It is difficult to frog and can obscure stitches.",["luxury","garment"]],
    ["sock-yarn","Sock yarn blend",["Socks","Shawls","Light garments","Colourwork"],"Usually high-twist wool plus nylon for recovery and wear.","Check washability and fibre content, not the label name alone.",["sock","easy care"]]
  ].map(([id,name,bestFor,shortExplanation,warning,tags])=>m({id,name,category:"Blends",bestFor,shortExplanation,behaviour:shortExplanation,warning,tags,durability:"high"}));
  const structures=[
    ["single-ply","Single-ply yarn","Soft and round with one twisted strand.","Can pill and abrade faster.",["Simple stitches","Soft accessories"]],
    ["two-ply","2-ply yarn","Two strands create a light, flexible yarn.","Suitability depends strongly on twist.",["Lace","Delicate fabric"]],
    ["multi-ply","3-ply / multi-ply yarn","Balanced plies often improve strength and stitch definition.","Very firm yarn can reduce drape.",["Garments","Cables","Texture"]],
    ["chainette","Chainette yarn","A tiny knitted tube traps air, giving thickness without heavy weight.","Check snagging and fibre content.",["Sweaters","Cardigans","Light chunky projects"]],
    ["roving","Roving yarn","Lofty, lightly twisted fibre with a soft handmade look.","Pills, sheds and felts easily.",["Statement accessories","Felting"]],
    ["boucle","Bouclé yarn","Loops create bumpy texture and hide individual stitches.","Hard to count and frog; not beginner-friendly.",["Scarves","Simple shapes","Statement garments"]],
    ["chenille","Chenille / velvet yarn","Plush pile creates very soft, thick fabric.","Can worm or shed; stitches and ends are hard to see.",["Plushies","Blankets","Soft toys"]],
    ["eyelash","Eyelash / novelty yarn","Fuzzy strands add decorative texture.","Hides detail and is difficult to frog.",["Trims","Costumes","Texture effects"]],
    ["ribbon","Ribbon yarn","Flat tape produces distinctive texture and drape.","The tape may twist while working.",["Bags","Accessories","Summer items"]],
    ["tshirt","T-shirt yarn","Heavy fabric strips make sturdy, thick material.","Heavy and hard on hands.",["Baskets","Rugs","Bags"]],
    ["raffia","Raffia / paper yarn","Firm, low-stretch material for crisp structure.","Can feel rough and tiring to work.",["Summer hats","Bags","Home decor"]]
  ].map(([id,name,shortExplanation,warning,bestFor])=>m({id,name,category:"Yarn structure",bestFor,shortExplanation,behaviour:shortExplanation,warning,beginnerFriendly:!/[roving|bouclé|chenille|eyelash]/i.test(name),tags:["structure",...(bestFor.includes("Bags")?["bag"]:[])],relatedTools:["Needle / Hook Adjustment","Gauge / Swatch Adapter","Yarn Substitution Helper"]}));
  const guide=(id,name,category,shortExplanation,warning,bestFor,tags=[],relatedTools=["Yarn Substitution Helper","Gauge / Swatch Adapter"])=>m({id,name,category,bestFor,shortExplanation,behaviour:shortExplanation,warning,notIdealFor:[],tags,relatedTools});
  const projects=[
    ["sweaters","Best yarns for sweaters","Wool, merino and balanced wool blends give recovery; cotton blends suit summer.","Pure cotton can grow, pure alpaca can sag, and superwash can stretch.",["Wool","Merino","Alpaca wool blend","Wool silk blend","Cotton blends"]],
    ["cardigans","Best yarns for cardigans","Wool blends, alpaca-wool, cotton-acrylic and chainette balance drape and structure.","Check total garment weight and button-band stretch.",["Wool","Wool acrylic blend","Chainette yarn"]],
    ["socks","Best yarns for socks","High-twist wool with nylon gives warmth, recovery and abrasion resistance.","Avoid pure alpaca, cotton and delicate luxury fibre unless designed for socks.",["Wool nylon blend","Sock yarn blend","Superwash wool with nylon"]],
    ["baby","Best yarns for baby items","Choose soft, low-shedding yarn that can handle realistic washing.","Avoid shedding fibres and hard-to-wash yarn; always check the label.",["Superwash merino","Premium acrylic","Cotton acrylic blend","Soft cotton"]],
    ["amigurumi","Best yarns for amigurumi","Smooth cotton and acrylic show stitches; chenille suits plush toys.","Fuzzy yarn hides small details and chenille stitches can be hard to see.",["Cotton","Mercerized cotton","Acrylic","Chenille"]],
    ["bags","Best yarns for bags","Low-stretch, durable cotton, hemp, linen, T-shirt yarn and raffia hold shape.","Springy wool may not keep a bag structured.",["Cotton","Hemp","Linen","T-shirt yarn","Raffia"]],
    ["blankets","Best yarns for blankets","Easy-care acrylic and blends are practical; superwash wool adds warmth.","Consider finished weight and realistic washability.",["Acrylic","Wool acrylic blend","Cotton acrylic blend","Chenille"]],
    ["summer-tops","Best yarns for summer tops","Cotton, linen, bamboo, Tencel and silk blends feel cool and drapey.","Plant fibres have little recovery and may grow.",["Cotton","Linen","Bamboo","Tencel"]],
    ["shawls","Best yarns for shawls","Choose wool for warmth, silk or bamboo for drape, and mohair for halo.","Match the fibre to the needed warmth, lace definition and drape.",["Wool","Silk blends","Alpaca","Mohair silk"]],
    ["colourwork","Best yarns for colourwork","Grippy wool and smooth blends help motifs stay clear.","Slippery plant fibres grip less and fuzzy yarn blurs motifs.",["Wool","Non-superwash wool","Smooth wool blends"]],
    ["cables","Best yarns for cables","Smooth multi-ply wool gives bounce and crisp shadows.","Fuzzy, dark or busy variegated yarn can hide cables.",["Smooth wool","Merino","Multi-ply yarn"]]
  ].map(([id,name,shortExplanation,warning,bestFor])=>guide(`project-${id}`,name,"Project guides",shortExplanation,warning,bestFor,[id,"garment"]));
  const warnings=[
    ["grow","Yarn can grow after blocking","Superwash wool, alpaca, cotton, bamboo, silk and heavy garments may relax or stretch.","Wash and hang a representative swatch; compare stitch and row gauge.",["Gauge / Swatch Adapter","Garment Resizer"]],
    ["same-weight","Same yarn weight does not mean same result","Two DK yarns can differ in fibre, twist, ply, density and drape.","Compare gauge, fibre, metres per 100 g, stitch definition and a washed swatch.",["Yarn Substitution Helper"]],
    ["pilling","Soft yarn may pill faster","Fine fibres and loose twist move more under friction.","Consider underarms, cuffs, sides, bags and the yarn's twist.",["Yarn Materials"]],
    ["heavy","Heavy yarn can distort garments","Cotton, bamboo, alpaca and oversized pieces can pull shoulders and length downward.","Check weight, drape, shoulder structure and a hanging swatch.",["Garment Resizer","Gauge / Swatch Adapter"]],
    ["fuzzy","Fuzzy yarn hides stitch detail","Mohair, alpaca, angora, bouclé and novelty yarn blur lace, cables and texture.","Choose a smooth yarn when stitch definition or easy frogging matters.",["Yarn Substitution Helper","OG Chart Mode"]],
    ["slippery","Slippery yarn can be harder to control","Silk, bamboo, Tencel and mercerized cotton may slide on tools.","Adjust tool material, tension and edge security.",["Needle / Hook Adjustment"]]
  ].map(([id,name,shortExplanation,warning,relatedTools])=>guide(`warning-${id}`,name,"Behaviour warnings",shortExplanation,warning,[],["warning"],relatedTools));
  const care=[
    ["machine","Machine washable yarn","Practical for baby items, gifts, blankets and everyday garments.","Machine washable does not always mean dryer-safe."],
    ["hand","Hand wash yarn","Delicate fibres need gentle washing and flat drying.","Avoid when the recipient needs simple machine care."],
    ["superwash","Superwash care","Washable wool may grow; a permitted dryer cycle can sometimes restore bounce.","Always follow the label before using a dryer."],
    ["blocking","Blocking by fibre","Wool responds to wet blocking; cotton and linen relax; acrylic needs cautious steam.","High heat can permanently damage acrylic and luxury fibres."],
    ["pilling","Pilling","Friction moves loose fibres at underarms, cuffs, sides and bags.","Very soft yarn may pill faster."],
    ["bleeding","Colour bleeding","Strong dyes can release colour and mark light sections.","Test colourwork yarns before washing them together."],
    ["storage","Storing yarn","Keep yarn dry, clean, labelled and protected from pests.","Animal fibres need moth protection."]
  ].map(([id,name,shortExplanation,warning])=>guide(`care-${id}`,name,"Care",shortExplanation,warning,[],["easy care"],["Blocking Calculator","Project Notes"]));
  const substitutions=[
    ["fibre","Substituting by fibre","Changing fibre changes drape, stretch, warmth and care."],
    ["yardage","Substituting by yardage","Compare total metres or yards, not ball weight alone."],
    ["gauge","Substituting by gauge","The replacement must make the right fabric at the required gauge."],
    ["garments","Substituting for garments","Fit depends on recovery, drape, weight and washed gauge."],
    ["amigurumi","Substituting for amigurumi","Different weights work, but finished toy size and stuffing density change."],
    ["bags","Substituting for bags","Prioritise structure, durability and low stretch."],
    ["baby","Substituting for baby items","Prioritise softness, washability, safety and low shedding."]
  ].map(([id,name,shortExplanation])=>guide(`sub-${id}`,name,"Substitution",shortExplanation,"Make and wash a swatch before committing.",[],[id],["Yarn Substitution Helper","Yarn Weight Converter","Gauge / Swatch Adapter"]));
  return [...fibres,...blends,...structures,...projects,...warnings,...care,...substitutions];
}
...
function openProjectIdeaModal(ideaId=null){
  ...
  openModal(`<p class="eyebrow">PROJECT IDEA</p><h2>${idea?"Edit":"Add"} project idea</h2>
    <div class="form-grid">
      <div class="field full"><label for="idea-title">Idea title</label><input id="idea-title" value="${escapeHtml(idea?.title||"")}" placeholder="e.g. Planned pooling scarf"></div>
      <div class="field"><label for="idea-craft">Craft type</label><select id="idea-craft">${ideaCraftOptions.map(v=>`<option ${idea?.craftType===v?"selected":""}>${escapeHtml(v)}</option>`).join("")}</select></div>
      <div class="field"><label for="idea-kind">Project kind</label><select id="idea-kind">${ideaKindOptions.map(v=>`<option ${idea?.projectKind===v?"selected":""}>${escapeHtml(v)}</option>`).join("")}</select></div>
      <div class="field"><label for="idea-platform">Source platform</label><select id="idea-platform">${ideaPlatformOptions.map(v=>`<option ${idea?.sourcePlatform===v?"selected":""}>${escapeHtml(v)}</option>`).join("")}</select></div>
      <div class="field"><label for="idea-difficulty">Difficulty guess</label><select id="idea-difficulty">${ideaDifficultyOptions.map(v=>`<option ${idea?.difficultyGuess===v?"selected":""}>${escapeHtml(v)}</option>`).join("")}</select></div>
      <div class="field full"><label for="idea-source-link">Source link</label><input id="idea-source-link" value="${escapeHtml(idea?.sourceLink||"")}" placeholder="Optional URL"></div>
      <div class="field full upload-drop"><label for="idea-image">Reference image</label><input id="idea-image" type="file" accept="image/*"><small>${idea?.referenceImageAsset?"Choose a new photo to replace the saved reference.":"Optional photo or screenshot."}</small></div>
      <div class="field full"><label for="idea-notes">Inspiration notes</label><textarea id="idea-notes" rows="5" placeholder="What do you like about this idea?">${escapeHtml(idea?.inspirationNotes||idea?.description||"")}</textarea></div>
      <div class="field full"><label for="idea-yarn">Yarn estimate</label><input id="idea-yarn" value="${escapeHtml(idea?.yarnEstimate||"")}" placeholder="Optional yarn amount or note"></div>
      <div class="field full"><label for="idea-tags">Tags</label><input id="idea-tags" value="${escapeHtml((idea?.tags||[]).join(", "))}" placeholder="cozy, cardigan, gift"></div>
    </div>
...
const symbolSvgPaths={
  knit:'<path d="M10 32h44"></path>',
  purl:'<path d="M32 10v44"></path>',
  slip:'<path d="M15 16 32 50 49 16"></path>',
  "yarn-over":'<circle cx="32" cy="32" r="17"></circle>',
  "double-yarn-over":'<circle cx="23" cy="32" r="12"></circle><circle cx="41" cy="32" r="12"></circle>',
  "knit-twisted":'<ellipse cx="32" cy="27" rx="9" ry="15"></ellipse><path d="M32 42c-5 8-13 10-20 5M32 42c5 8 13 10 20 5"></path>',
  "purl-twisted":'<ellipse cx="32" cy="25" rx="9" ry="14"></ellipse><path d="M32 39c-5 7-13 9-20 5M32 39c5 7 13 9 20 5M19 54h26"></path>',
  increase:'<path d="M12 14 32 52 52 14"></path>',
  "increase-kfb":'<path d="M32 54V31M32 31 18 12M32 31 46 12M18 12h10"></path>',
  "increase-left":'<path d="M48 52V14M48 34 16 14"></path>',
  "increase-right":'<path d="M16 52V14M16 34 48 14"></path>',
  ...
  "purl-decrease-right":'<path d="m16 52 32-40M25 53h22"></path>',
  "purl-decrease-left":'<path d="m16 12 32 40M17 53h22"></path>',
  ...
  "front-loop":'<path d="M10 25c11 22 33 22 44 0"></path>',
  "back-loop":'<path d="M10 39c11-22 33-22 44 0"></path>',
  ...
  "cable-left":'<path d="M14 12 50 52"></path><path class="symbol-svg-under" d="M25 39 39 25"></path><path d="M14 52 50 12"></path>',
  "cable-right":'<path d="M14 52 50 12"></path><path class="symbol-svg-under" d="M25 25 39 39"></path><path d="M14 12 50 52"></path>',
  "cable-left-generic":'<path d="M12 12c9 0 13 8 20 20s11 20 20 20M12 52c9 0 13-8 20-20s11-20 20-20"></path><path d="M10 12h8M46 12h8"></path>',
  "cable-right-generic":'<path d="M12 52c9 0 13-8 20-20s11-20 20-20M12 12c9 0 13 8 20 20s11 20 20 20"></path><path d="M10 52h8M46 52h8"></path>',
  "cable-left-wide":'<path d="M8 12 44 52M20 12 56 52"></path><path class="symbol-svg-under" d="M8 52 44 12M20 52 56 12"></path><path d="M8 52 44 12M20 52 56 12"></path>',
  "cable-right-wide":'<path d="M8 52 44 12M20 52 56 12"></path><path class="symbol-svg-under" d="M8 12 44 52M20 12 56 52"></path><path d="M8 12 44 52M20 12 56 52"></path>',
  "cable-left-3-3":'<path d="M4 10 38 54M14 10 48 54M24 10 58 54"></path><path class="symbol-svg-under" d="M4 54 38 10M14 54 48 10M24 54 58 10"></path><path d="M4 54 38 10M14 54 48 10M24 54 58 10"></path>',
  "cable-right-3-3":'<path d="M4 54 38 10M14 54 48 10M24 54 58 10"></path><path class="symbol-svg-under" d="M4 10 38 54M14 10 48 54M24 10 58 54"></path><path d="M4 10 38 54M14 10 48 54M24 10 58 54"></path>',
  "cable-left-4-4":'<path d="M2 8 30 56M12 8 40 56M22 8 50 56M32 8 60 56"></path><path class="symbol-svg-under" d="M2 56 30 8M12 56 40 8M22 56 50 8M32 56 60 8"></path><path d="M2 56 30 8M12 56 40 8M22 56 50 8M32 56 60 8"></path>',
  "cable-right-4-4":'<path d="M2 56 30 8M12 56 40 8M22 56 50 8M32 56 60 8"></path><path class="symbol-svg-under" d="M2 8 30 56M12 8 40 56M22 8 50 56M32 8 60 56"></path><path d="M2 8 30 56M12 8 40 56M22 8 50 56M32 8 60 56"></path>',
  "cable-left-purl":'<path d="M14 12 50 52"></path><path class="symbol-svg-under" d="M25 39 39 25"></path><path d="M14 52 50 12M20 56h24"></path>',
  "cable-right-purl":'<path d="M14 52 50 12"></path><path class="symbol-svg-under" d="M25 25 39 39"></path><path d="M14 12 50 52M20 56h24"></path>',
  ...
  "tunisian-simple":'<path d="M10 8h44v48H10Z"></path><path stroke-dasharray="3 3" d="M10 25h44M32 25v25"></path><path d="M20 14c8-5 16 5 24 0"></path>',
  "tunisian-purl":'<path d="M10 8h44v48H10Z"></path><path stroke-dasharray="3 3" d="M10 25h44"></path><path d="M18 14c9-5 19 5 28 0M20 43h24"></path>',
  "tunisian-knit":'<path d="M10 8h44v48H10Z"></path><path stroke-dasharray="3 3" d="M10 25h44"></path><path d="M18 14c9-5 19 5 28 0M32 49c-14-8-10-21 0-21s14 13 0 21Z"></path>',
  "tunisian-reverse":'<path d="M10 8h44v48H10Z"></path><path stroke-dasharray="3 3" d="M10 25h44M32 25v21"></path><path d="M18 14c9-5 19 5 28 0M21 47c6 8 16 8 22 0"></path>',
  "tunisian-full":'<path d="M10 8h44v48H10Z"></path><path stroke-dasharray="3 3" d="M10 25h44"></path><path d="M18 14c9-5 19 5 28 0"></path><circle cx="32" cy="40" r="8"></circle>',
  "tunisian-double":'<path d="M10 8h44v48H10Z"></path><path stroke-dasharray="3 3" d="M10 25h44M32 25v25M25 40l14-8"></path><path d="M18 14c9-5 19 5 28 0"></path>',
  "tunisian-slip":'<path d="M10 8h44v48H10Z"></path><path stroke-dasharray="3 3" d="M10 25h44"></path><path d="M18 14c9-5 19 5 28 0M20 50 32 29l12 21"></path>',
  "tunisian-yarn-over":'<path d="M10 8h44v48H10Z"></path><path stroke-dasharray="3 3" d="M10 25h44"></path><path d="M18 14c9-5 19 5 28 0"></path><ellipse cx="32" cy="40" rx="11" ry="7"></ellipse>',
  "tunisian-yarn-over-space":'<path d="M10 8h44v48H10Z"></path><path stroke-dasharray="3 3" d="M10 25h44"></path><path d="M18 14c9-5 19 5 28 0M20 49c7-18 17-18 24 0"></path><circle cx="32" cy="37" r="5"></circle>',
  "tunisian-increase-1-3":'<path d="M7 8h50v48H7Z"></path><path stroke-dasharray="3 3" d="M7 25h50"></path><path d="M16 14c7-5 13 5 20 0 7-5 12 5 18 0M32 50V35M32 35 19 26M32 35l13-9"></path><circle cx="32" cy="39" r="5"></circle>',
  "tunisian-decrease-2":'<path d="M7 8h50v48H7Z"></path><path stroke-dasharray="3 3" d="M7 25h50"></path><path d="M16 14c7-5 13 5 20 0 7-5 12 5 18 0M15 49 32 29l17 20"></path>',
  "tunisian-decrease-3":'<path d="M5 8h54v48H5Z"></path><path stroke-dasharray="3 3" d="M5 25h54"></path><path d="M12 14c6-5 11 5 17 0s11 5 17 0 8 4 12 0M11 49 32 29l21 20M32 29v20"></path>',
  "tunisian-decrease-4":'<path d="M4 8h56v48H4Z"></path><path stroke-dasharray="3 3" d="M4 25h56"></path><path d="M8 14c6-5 11 5 17 0s11 5 17 0 11 5 17 0M7 49 32 29l25 20M20 49l12-20M44 49 32 29"></path>',
  "tunisian-decrease-5":'<path d="M3 8h58v48H3Z"></path><path stroke-dasharray="3 3" d="M3 25h58"></path><path d="M6 14c5-5 10 5 15 0s10 5 15 0 10 5 15 0 7 4 10 0M5 49 32 29l27 20M18 49l14-20M46 49 32 29M32 29v20"></path>',
  "tunisian-cross-a":'<path d="M7 8h50v48H7Z"></path><path stroke-dasharray="3 3" d="M7 25h50"></path><path d="M16 14c7-5 13 5 20 0 7-5 12 5 18 0M13 50 49 29M15 29l34 21"></path>',
  "tunisian-cross-b":'<path d="M7 8h50v48H7Z"></path><path stroke-dasharray="3 3" d="M7 25h50"></path><path d="M16 14c7-5 13 5 20 0 7-5 12 5 18 0M15 29l34 21M13 50 49 29"></path><path d="M36 30v10"></path>',
  "tunisian-double-cross":'<path d="M7 8h50v48H7Z"></path><path stroke-dasharray="3 3" d="M7 25h50"></path><path d="M16 14c7-5 13 5 20 0 7-5 12 5 18 0M14 50 47 29M17 29l33 21M23 39l11-7M34 46l11-7"></path>',
  "tunisian-cable-left-3":'<path d="M3 8h58v48H3Z"></path><path stroke-dasharray="3 3" d="M3 25h58"></path><path d="M7 14c5-5 10 5 15 0s10 5 15 0 10 5 15 0 6 4 9 0M5 48 35 29M16 48l30-19M27 48l30-19"></path>',
  cluster:'<path d="M16 52 27 18M32 52V18M48 52 37 18M25 18h14M20 38l11-6M33 32l11 6"></path><path d="M25 14h14"></path>',
  "cluster-increase":'<path d="M32 54 13 18M32 54 25 16M32 54 39 16M32 54 51 18M8 18h10M20 16h10M34 16h10M46 18h10M17 36l12-6M35 30l12 6"></path><circle class="symbol-svg-fill" cx="32" cy="54" r="2.5"></circle>',
  ...
  popcorn:'<path d="M32 52 12 20M32 52 22 17M32 52V16M32 52 42 17M32 52 52 20M9 33l10-6M18 30l10-6M27 29l10-6M36 30l10-6M45 33l8-5"></path><ellipse cx="32" cy="14" rx="20" ry="6"></ellipse>',
  shell:'<path d="M32 54 10 20M32 54 21 17M32 54V16M32 54 43 17M32 54 54 20M7 20h12M16 17h12M26 16h12M36 17h12M45 20h12M12 36l10-6M22 32l9-5M33 27l9 5M42 30l10 6"></path>',
  "v-stitch":'<path d="M32 54 17 15M32 54 47 15M8 15h18M38 15h18M13 36l12-7M39 29l12 7"></path><ellipse cx="32" cy="48" rx="5" ry="3"></ellipse>',
  "y-stitch":'<path d="M32 56V32M32 32 15 12M32 32 49 12M23 43l18-9M10 12h12M42 12h12M19 27l12-7M33 20l12 7"></path>',
  "crochet-cross":'<path d="M16 51 45 15M48 51 19 15M10 15h18M36 15h18M23 37l13-8M28 29l13 8"></path>',
  picot:'<ellipse cx="32" cy="17" rx="10" ry="6"></ellipse><path d="M22 18c-10 4-9 23 0 31 7-6 8-17 5-27M42 18c10 4 9 23 0 31-7-6-8-17-5-27"></path>',
  ...
  "legend-specific":'<path d="M13 10h38a4 4 0 0 1 4 4v36a4 4 0 0 1-4 4H13a4 4 0 0 1-4-4V14a4 4 0 0 1 4-4ZM17 21h30M17 31h20M17 41h26"></path>'
};
const symbolVerificationOptions=["To Be Confirmed","Confirmed","Manually Verified"];
const publicSymbolStatusOptions=["All","Verified only","Common symbols","Varies by chart key","Abbreviation only"];
function symbolPublicStatus(entry={}){
  if(["verified","commonNotUniversal","variesByDesigner","needsReview"].includes(entry.symbolStatus))return entry.symbolStatus;
  if(entry.verificationStatus==="Manually Verified")return"verified";
  if(entry.verificationStatus==="Confirmed")return"commonNotUniversal";
  return entry.symbolType==="legend-specific"||entry.symbolType==="chart-rule"?"variesByDesigner":"needsReview";
}
function symbolStatusCopy(entry={}){
  const status=symbolPublicStatus(entry);
  return status==="verified"?{label:"Verified",className:"verified",warning:"This chart mark is widely used and has been checked. Still confirm the pattern key."}:status==="commonNotUniversal"?{label:"Common, check key",className:"common",warning:"Common symbol, but check your pattern key."}:status==="needsReview"?{label:"Coming soon",className:"coming",warning:"This entry is being reviewed. Use the written abbreviation and your pattern key."}:{label:"Varies by chart key",className:"varies",warning:"Chart symbol varies by designer. Always check the pattern’s symbol key before following a chart."};
}
...
function symbolIconSvg(entry,className="symbol-svg-icon"){
  ...
  return `<svg class="${className}" viewBox="0 0 64 64" role="img" aria-label="${escapeHtml(entry.nameEn||entry.fullName||"Chart symbol")}">${icon}</svg>`;
}
function neutralSymbolHtml(entry,size="card"){
  const copy=symbolPublicStatus(entry)==="needsReview"?"Coming soon":entry.displayMode==="abbreviationOnly"?"Abbreviation":"Check key";
  return `<span class="symbol-neutral symbol-neutral-${size}" role="img" aria-label="${escapeHtml(entry.nameEn||"Symbol")} — ${escapeHtml(copy)}"><svg viewBox="0 0 64 64" aria-hidden="true"><circle cx="27" cy="27" r="15"></circle><path d="m38 38 13 13M22 24c1-5 10-6 12-1 2 6-6 6-6 12M28 42h.01"></path></svg><small>${escapeHtml(copy)}</small></span>`;
}
function symbolVisualHtml(entry,size="card"){
  const status=symbolPublicStatus(entry),canShow=status==="verified"||status==="commonNotUniversal";
  if(!canShow)return neutralSymbolHtml(entry,size);
...
function symbolVerificationBadge(entry){
  const status=symbolStatusCopy(entry);
  return `<span class="symbol-verification-badge ${status.className}"><i aria-hidden="true"></i>${escapeHtml(status.label)}</span>`;
}
...
function symbolDatabaseHtml(){
  ...
  if(currentSymbolId){
    ...
    return `<div class="symbol-detail-overlay" role="dialog" aria-modal="true" aria-label="${escapeHtml(entry.nameEn)} symbol details"><section class="symbol-detail card">
      <button class="modal-close" id="symbol-detail-close" aria-label="Close symbol details">${uiIcon("x","ui-icon")}</button><button class="text-button symbol-detail-back" id="symbol-detail-back">← Symbol Database</button>
      <div class="symbol-detail-hero"><div class="symbol-glyph">${symbolVisualHtml(entry,"detail")}</div><div><p class="eyebrow">${escapeHtml(entry.craft)} · ${escapeHtml(entry.referenceCategory||entry.category)} · ${escapeHtml(entry.difficulty)}</p><h2>${escapeHtml(entry.nameEn)}</h2><p class="symbol-chinese-name">${escapeHtml(entry.chineseName||entry.nameZh||entry.nameTraditionalChinese||"Traditional Chinese name coming soon")}</p><div class="symbol-card-badges"><span>${escapeHtml(entry.craft)}</span><span>${escapeHtml(entry.referenceCategory||entry.category)}</span><span>${escapeHtml(entry.difficulty)}</span>${symbolVerificationBadge(entry)}</div><div class="symbol-abbreviation-block" aria-label="Abbreviations"><h3>Abbreviations</h3><div class="symbol-abbreviation-chips ${["Crochet","Tunisian"].includes(entry.craft)?"crochet-abbreviation-chips":""}">${["Crochet","Tunisian"].includes(entry.craft)?`<span><small>US</small><strong>${escapeHtml(abbreviations.us||entry.abbreviationUS||"—")}</strong></span><span><small>UK</small><strong>${escapeHtml(abbreviations.uk||entry.abbreviationUK||"—")}</strong></span>`:`<span><small>UK/US</small><strong>${escapeHtml(abbreviations.usUk||"—")}</strong></span>`}<span><small>CN</small><strong>${escapeHtml(abbreviations.cn||"—")}</strong></span><span><small>JP</small><strong>${escapeHtml(abbreviations.jp||"—")}</strong></span></div></div></div></div>
      ...
      <div class="symbol-detail-grid">
        ...
        <article><h3>Common mistake</h3><p>${escapeHtml(entry.commonMistakes?.[0]||"Assuming the symbol is universal instead of checking the pattern legend.")}</p></article>
        <article><h3>Counting tip</h3><p>${escapeHtml(entry.countingTip||"Count your stitches after this row or round.")}</p></article>
        <article><h3>Example pattern wording</h3><p>${escapeHtml(entry.examplePatternWording||`${entry.abbreviation||entry.nameEn} as directed.`)}</p></article>
        <article><h3>Related symbols</h3><div class="related-symbol-links">${relatedEntries.length?relatedEntries.map(item=>item.match?`<button type="button" data-related-symbol="${escapeHtml(item.match.id)}"><strong>${escapeHtml(item.match.abbreviation||item.label)}</strong><small>${escapeHtml(item.match.nameEn)}</small></button>`:`<span>${escapeHtml(item.label)}</span>`).join(""):"<p>Use craft and category filters to find related entries.</p>"}</div></article>
      </div>
      <p class="symbol-global-warning">${escapeHtml(symbolStatusCopy(entry).warning)}</p>
      ${entry.craft==="Crochet"?`<p class="symbol-cn-warning">${escapeHtml(entry.cnChartWarning)}</p>`:""}
      ${entry.craft==="Crochet"?`<p class="symbol-terminology-warning">${escapeHtml(entry.usUkWarning)}</p>`:""}
      <details class="symbol-full-explanation"><summary>Full explanation</summary>${entry.craft==="Tunisian"?`<div class="symbol-full-explanation-grid"><section><h3>How to work it</h3><p>${escapeHtml(entry.howToWork)}</p></section><section><h3>Pass</h3><p>${escapeHtml(entry.passType)}</p></section><section><h3>Why it matters</h3><p>${escapeHtml(entry.whyItMatters)}</p></section><section><h3>Beginner tip</h3><p>${escapeHtml(entry.beginnerNote)}</p></section><section><h3>Common mistakes</h3><ul>${entry.commonMistakes.map(item=>`<li>${escapeHtml(item)}</li>`).join("")}</ul></section><section><h3>Counting tip</h3><p>${escapeHtml(entry.countingTip)}</p></section><section><h3>Example pattern wording</h3><p>${escapeHtml(entry.examplePatternWording)}</p></section><section><h3>Related tools</h3><ul>${(entry.relatedTools||["OG Chart Mode","Row / Round Counter Helper","Annotation Tool"]).map(item=>`<li>${escapeHtml(item)}</li>`).join("")}</ul></section></div>`:`<div class="symbol-full-explanation-grid"><section><h3>How to work it</h3><p>${escapeHtml(entry.howToWork||entry.howTo)}</p></section><section><h3>${entry.craft==="Crochet"?"Row chart note":"Flat knitting note"}</h3><p>${escapeHtml(entry.craft==="Crochet"?entry.rowChartNote:entry.flatChartNote||"Check right-side and wrong-side chart directions before working the row.")}</p></section><section><h3>${entry.craft==="Crochet"?"Round chart note":"In-the-round note"}</h3><p>${escapeHtml(entry.roundChartNote||"Read each round in the direction stated by the chart.")}</p></section><section><h3>Beginner tip</h3><p>${escapeHtml(entry.beginnerNote||entry.beginnerExplanation)}</p></section><section><h3>Common mistakes</h3><ul>${entry.commonMistakes.map(item=>`<li>${escapeHtml(item)}</li>`).join("")}</ul></section><section><h3>Related tools</h3><ul>${(entry.relatedTools||["OG Chart Mode","Row / Round Counter Helper","Annotation Tool"]).map(item=>`<li>${escapeHtml(item)}</li>`).join("")}</ul></section></div>`}</details>
  ...
  let entries=database.searchEntries(allEntries,symbolFilters.search,{...symbolFilters,verification:"All"});
  if(symbolFilters.referenceCategory!=="All")entries=entries.filter(entry=>entry.referenceCategory===symbolFilters.referenceCategory);
  if(symbolFilters.passType!=="All")entries=entries.filter(entry=>entry.passType===symbolFilters.passType);
  if(symbolFilters.verification==="Verified only")entries=entries.filter(entry=>symbolPublicStatus(entry)==="verified");
  if(symbolFilters.verification==="Common symbols")entries=entries.filter(entry=>symbolPublicStatus(entry)==="commonNotUniversal");
  if(symbolFilters.verification==="Varies by chart key")entries=entries.filter(entry=>symbolPublicStatus(entry)==="variesByDesigner");
  if(symbolFilters.verification==="Abbreviation only")entries=entries.filter(entry=>entry.displayMode==="abbreviationOnly");
  if(symbolFilters.sort==="Verification status"){
    const rank={verified:0,commonNotUniversal:1,variesByDesigner:2,needsReview:3};
    entries=[...entries].sort((a,b)=>(rank[symbolPublicStatus(a)]??4)-(rank[symbolPublicStatus(b)]??4)||a.nameEn.localeCompare(b.nameEn));
  }
  ...
  const craftOptions=["All","Knitting","Crochet","Tunisian","Shared"],categoryOptions=["All",...database.categoryOrder],difficultyOptions=["All","Beginner","Intermediate","Advanced"],terminologyOptions=["All","US","UK","CN","Chart Symbol"],verificationOptions=publicSymbolStatusOptions,sortOptions=["Default","Verification status"];
  const crochetMode=symbolFilters.craft==="Crochet",tunisianMode=symbolFilters.craft==="Tunisian";
  const quickFilters=tunisianMode?["All","Tunisian Crochet Basics","Core Tunisian Stitches","Tunisian Increases","Tunisian Decreases","Return Pass Variations","Colourwork & Special Techniques","Chart Reading Notes","Beginner Confusion Notes"]:crochetMode?["All","Beginner","Intermediate","Advanced","Basic","Chain","Slip Stitch","Increase","Decrease","Loop Placement","Post Stitch","Shell","Texture","Lace","Colourwork","Amigurumi","Chart Structure","Finishing"]:["All","Beginner","Intermediate","Advanced","Basic","Increase","Decrease","Lace","Cable","Texture","Colourwork","Short Row","Chart Structure","Construction"];
  return `<section class="symbol-database-shell">
    ...
    <div class="symbol-reference-switch" aria-label="Symbol reference craft"><button class="secondary-button" data-symbol-craft-mode="Knitting">Knitting</button><button class="secondary-button" data-symbol-craft-mode="Crochet">Crochet</button><button class="secondary-button" data-symbol-craft-mode="Tunisian">Tunisian crochet</button><button class="secondary-button" data-symbol-craft-mode="All">All crafts</button></div>
    ${crochetMode?`<div class="crochet-reading-guide card"><p class="eyebrow">CROCHET CHART READING BASICS</p><div><section><h3>Rows</h3><p>Usually alternate direction. Turning chains may or may not count as stitches.</p></section><section><h3>Rounds & motifs</h3><p>Read from the centre outward and watch round numbers, corners, spaces, and repeats.</p></section><section><h3>Amigurumi</h3><p>Many patterns use continuous rounds. Mark the beginning of round and keep an accurate stitch count.</p></section></div><p class="symbol-terminology-warning">US and UK crochet terms are different. The same stitch name can mean different stitches. Always check whether your pattern uses US or UK terms.</p></div>`:""}
    ${tunisianMode?`<div class="tunisian-reading-guide card"><p class="eyebrow">TUNISIAN TWO-PASS BASICS</p><p>A Tunisian row normally has two parts: the Forward Pass picks up loops and keeps them on the hook; the Return Pass works those loops off. Most patterns count both passes as one row.</p><div><strong>Count after FwP</strong><span>Count every loop on the hook, including the first loop.</span><strong>Count after RetP</strong><span>Confirm the finished stitch count and work the last stitch under two side loops.</span></div></div>`:""}
    <div class="symbol-quick-filters" aria-label="${tunisianMode?"Tunisian crochet":crochetMode?"Crochet":"Knitting"} symbol filters">${quickFilters.map(value=>`<button class="secondary-button" data-symbol-quick="${value}">${value==="Short Row"?"Short rows":value==="Chart Structure"?"Chart structure":value==="Slip Stitch"?"Slip stitch":value==="Loop Placement"?"Loop placement":value==="Post Stitch"?"Post stitch":value}</button>`).join("")}</div>
    <div class="symbol-filter-bar card"><label class="symbol-search"><span>Search</span><input id="symbol-search" type="search" value="${escapeHtml(symbolFilters.search)}" placeholder="Symbol, abbreviation, 中文名稱 or alias"></label><label><span>Craft</span><select id="symbol-craft-filter">${craftOptions.map(value=>`<option ${symbolFilters.craft===value?"selected":""}>${value}</option>`).join("")}</select></label><label><span>Category</span><select id="symbol-category-filter">${categoryOptions.map(value=>`<option ${symbolFilters.category===value?"selected":""}>${value}</option>`).join("")}</select></label><label><span>Difficulty</span><select id="symbol-difficulty-filter">${difficultyOptions.map(value=>`<option ${symbolFilters.difficulty===value?"selected":""}>${value}</option>`).join("")}</select></label>${tunisianMode?`<label><span>Pass</span><select id="symbol-pass-filter">${["All","Forward Pass","Return Pass","Both"].map(value=>`<option ${symbolFilters.passType===value?"selected":""}>${value}</option>`).join("")}</select></label>`:""}<label><span>Terminology</span><select id="symbol-terminology-filter">${terminologyOptions.map(value=>`<option ${symbolFilters.terminology===value?"selected":""}>${value==="US"?"US terms":value==="UK"?"UK terms":value==="CN"?"Chinese terms":value}</option>`).join("")}</select></label><label><span>Verification</span><select id="symbol-verification-filter">${verificationOptions.map(value=>`<option ${symbolFilters.verification===value?"selected":""}>${value}</option>`).join("")}</select></label><label><span>Sort</span><select id="symbol-sort">${sortOptions.map(value=>`<option ${symbolFilters.sort===value?"selected":""}>${value}</option>`).join("")}</select></label></div>
    <p class="symbol-result-count">${entries.length} matching entries · ${(state.symbolFavorites||[]).length} favorites</p>
    ${entries.length?sections.map(section=>{const items=entries.filter(entry=>entry.section===section);return items.length?`<section class="symbol-section"><div class="section-heading"><div><p class="eyebrow">SYMBOL DATABASE</p><h2>${escapeHtml(section)}</h2></div><span>${items.length} entries</span></div><div class="symbol-grid">${items.map(entry=>`<article class="symbol-card card"><button class="symbol-card-open" data-symbol-id="${entry.id}" aria-label="Open ${escapeHtml(entry.nameEn)} details"><span class="symbol-card-mark">${symbolVisualHtml(entry)}</span><span class="symbol-card-copy"><strong>${escapeHtml(entry.nameEn)}</strong><small>${escapeHtml(entry.chineseName||entry.nameZh||entry.nameTraditionalChinese||"Traditional Chinese name coming soon")}</small><span class="symbol-card-abbreviation">${escapeHtml(entry.abbreviations?.usUk||entry.abbreviation||"Abbreviation not used")}</span><span class="symbol-card-badges"><span>${escapeHtml(entry.craft)}</span><span>${escapeHtml(entry.category)}</span><span>${escapeHtml(entry.difficulty)}</span>${symbolVerificationBadge(entry)}</span></span></button><button class="symbol-card-edit" data-edit-symbol="${entry.id}" aria-label="Edit ${escapeHtml(entry.nameEn)}" title="Edit symbol">${uiIcon("edit","ui-icon")}</button>${(state.symbolFavorites||[]).includes(entry.id)?`<span class="symbol-favorite" aria-label="Favorite">Saved</span>`:""}</article>`).join("")}</div></section>`:"";}).join(""):`<div class="empty-state"><h3>No symbols match these filters</h3><p>Try a different craft, category, terminology or search term.</p></div>`}
  </section>`;
}
...
function closeSymbolDetail(){
  const previous=currentSymbolId;
  currentSymbolId=null;
  renderLibrary();
  [...document.querySelectorAll("[data-symbol-id]")].find(button=>button.dataset.symbolId===previous)?.focus();
}
function bindSymbolDatabase(){
  ...
  [["symbol-craft-filter","craft"],["symbol-category-filter","category"],["symbol-difficulty-filter","difficulty"],["symbol-pass-filter","passType"],["symbol-terminology-filter","terminology"],["symbol-verification-filter","verification"],["symbol-sort","sort"]].forEach(([id,key])=>document.getElementById(id)?.addEventListener("change",event=>{symbolFilters[key]=event.target.value;renderLibrary();}));
  document.querySelectorAll("[data-symbol-quick]").forEach(button=>button.addEventListener("click",()=>{const value=button.dataset.symbolQuick,currentCraft=symbolFilters.craft;symbolFilters.craft=["Knitting","Crochet","Tunisian"].includes(currentCraft)?currentCraft:(value==="All"?"All":"Knitting");symbolFilters.difficulty=["Beginner","Intermediate","Advanced"].includes(value)?value:"All";symbolFilters.category=window.YarnchaSymbolDatabase.categoryOrder.includes(value)?value:"All";symbolFilters.referenceCategory=currentCraft==="Tunisian"&&value!=="All"?value:"All";renderLibrary();}));
  document.querySelectorAll("[data-symbol-craft-mode]").forEach(button=>button.addEventListener("click",()=>{symbolFilters.craft=button.dataset.symbolCraftMode;symbolFilters.category="All";symbolFilters.referenceCategory="All";symbolFilters.passType="All";symbolFilters.difficulty="All";symbolFilters.search="";renderLibrary();}));
  ...
  document.getElementById("symbol-detail-back")?.addEventListener("click",closeSymbolDetail);
  document.getElementById("symbol-detail-close")?.addEventListener("click",closeSymbolDetail);
  const detailOverlay=document.querySelector(".symbol-detail-overlay");
  if(detailOverlay){
    detailOverlay.addEventListener("click",event=>{if(event.target===detailOverlay)closeSymbolDetail();});
    document.getElementById("symbol-detail-close")?.focus({preventScroll:true});
  }
...
function renderLibrary() {
  ...
  if(!currentLibrarySection){
    ...
    document.querySelectorAll("[data-material-filter]").forEach(button=>button.onclick=()=>{yarnMaterialFilter=button.dataset.materialFilter;renderLibrary();});
    document.querySelectorAll("[data-material-tool]").forEach(button=>button.onclick=()=>{const match=toolkitToolDefs.find(tool=>tool.name===button.dataset.materialTool||tool.name.includes(button.dataset.materialTool.split(" ")[0]));if(match){showView("tools");renderTool(match.id);}else toast(`${button.dataset.materialTool} is in the Tool Manual.`);});
    bindProjectIdeas();
    bindSymbolDatabase();
    bindLibraryWiki();
...
function openLibrarySuggestEdit(entryId){
  ...
  if(!entry)return;
  openModal(`<p class="eyebrow">SUGGEST EDIT</p><h2>${escapeHtml(entry.title)}</h2><p class="muted-copy">Suggestions are stored locally for now. Future community review can submit these to moderators.</p><div class="field full"><label for="wiki-suggest-text">What should change?</label><textarea id="wiki-suggest-text" rows="6" placeholder="Add a correction, missing tip, clearer wording, or source note."></textarea></div><div class="modal-actions"><button class="secondary-button" onclick="closeModal()">Cancel</button><button class="primary-button" id="save-wiki-suggestion">Save suggestion</button></div>`);
...
let yarnMaterialFilter="All";
const yarnMaterialFilters=["All","Wool","Cotton","Acrylic","Plant fibre","Animal fibre","Synthetic","Luxury","Blends","Yarn structure","Easy care","Garment","Socks","Bags","Amigurumi","Baby","Summer","Winter","Sensitive skin","Beginner friendly"];
function yarnMaterialMatchesFilter(material,filter){
  if(filter==="All")return true;
  const text=[material.name,material.category,...(material.tags||[]),...(material.bestFor||[]),material.careLevel,material.beginnerFriendly?"beginner friendly":""].join(" ").toLowerCase();
  const aliases={"plant fibre":"plant fibres","animal fibre":"animal fibres",synthetic:"synthetic fibres",garment:"garment",socks:"sock",bags:"bag",baby:"baby","easy care":"easy care","yarn structure":"yarn structure"};
  return text.includes((aliases[filter.toLowerCase()]||filter).toLowerCase());
}
function materialLevelLabel(value){return String(value||"medium").replace(/^./,letter=>letter.toUpperCase());}
function yarnMaterialCardHtml(material){
  const best=(material.bestFor||[]).slice(0,4),warning=material.mainWarning||material.commonProblems||"Check the label and swatch before committing.";
  const list=(title,items)=>items?.length?`<section><h4>${title}</h4><ul>${items.map(item=>`<li>${escapeHtml(item)}</li>`).join("")}</ul></section>`:"";
  return `<details class="material-card card" data-material-id="${escapeHtml(material.id)}"><summary><div>${material.imageAsset?`<img class="material-photo" data-material-image="${escapeHtml(material.imageAsset)}" alt="">`:""}<p class="eyebrow">${escapeHtml(material.category||material.origin)}</p><h3 class="content-title">${escapeHtml(material.name)}</h3><div class="material-preview-badges"><span>${escapeHtml(material.careLevel||"moderate")} care</span>${material.beginnerFriendly?"<span>Beginner friendly</span>":"<span>More practice</span>"}</div><p>${escapeHtml(material.shortExplanation||material.features||"")}</p><p class="material-best-preview"><strong>Best for:</strong> ${escapeHtml(best.join(", ")||"Compare with your project needs")}</p><p class="material-warning"><strong>Watch for:</strong> ${escapeHtml(warning)}</p></div><span class="material-expand-label">Details</span></summary><div class="material-card-details"><div class="material-trait-grid">${[["Warmth",material.warmth],["Softness",material.softness],["Elasticity",material.elasticity],["Drape",material.drape],["Durability",material.durability],["Stitch definition",material.stitchDefinition]].map(([label,value])=>`<span><small>${label}</small><strong>${materialLevelLabel(value)}</strong></span>`).join("")}</div><section><h4>What it is and how it behaves</h4><p>${escapeHtml(material.behaviour||material.features||material.shortExplanation||"")}</p></section><div class="material-detail-columns">${list("Best uses",material.bestFor)}${list("Not ideal for",material.notIdealFor)}</div><section><h4>Common problems</h4><p>${escapeHtml(material.commonProblems||warning)}</p></section><section><h4>Care tips</h4><p>${escapeHtml(material.careTips||"Check the label and wash a swatch first.")}</p></section><section><h4>Substitution notes</h4><p>${escapeHtml(material.substitutionNotes||"Compare gauge, fibre behavior and yardage.")}</p></section><section><h4>Related Yarncha tools</h4><div class="wiki-chip-row">${(material.relatedTools||[]).map(tool=>`<button class="chip" data-material-tool="${escapeHtml(tool)}">${escapeHtml(tool)}</button>`).join("")}</div></section><div class="row-actions"><button class="mini-button" data-edit-material="${escapeHtml(material.id)}">Edit</button></div></div></details>`;
}
function yarnMaterialReferenceHtml(){
  const materials=state.yarnMaterials.filter(material=>yarnMaterialMatchesFilter(material,yarnMaterialFilter));
  return `<div class="material-reference"><div class="material-source-note">Fibre is only part of the story. Spin, blend, ply, gauge and washed fabric all affect the result.</div><nav class="material-filter-chips" aria-label="Filter yarn materials">${yarnMaterialFilters.map(filter=>`<button class="chip ${yarnMaterialFilter===filter?"active":""}" data-material-filter="${escapeHtml(filter)}" aria-pressed="${yarnMaterialFilter===filter}">${escapeHtml(filter)}</button>`).join("")}</nav><div class="material-result-count"><strong>${materials.length}</strong> ${materials.length===1?"card":"cards"}</div><div class="material-grid">${materials.map(yarnMaterialCardHtml).join("")}</div></div>`;
}
...
const castOnRelatedTools=["Cast-on Calculator","Gauge / Swatch Adapter","Pattern Resizer","Row Counter","Project Notes","Project Checklist"];
function castOnMethodEntry({id,title,level,difficulty=level,useFor,pros,cons,mistakes,fixes,aliases=[]}){
  const article=/^[aeiou]/i.test(difficulty)?"an":"a";
  return wikiEntry({id,title,category:"Knitting Knowledge",subcategory:"Cast-on Methods",craftTypes:["knitting"],level,difficulty,summary:`${title} is ${article} ${difficulty} knitting cast-on suited to ${useFor.join(", ")}.`,fullExplanation:`This method creates the starting edge before the first row or round. Its useful qualities are ${pros.join(", ")}; its trade-offs are ${cons.join(", ")}.`,whenToUse:`Use it for ${useFor.join(", ")}.`,whyItMatters:"The cast-on edge controls the opening's stretch, stability, appearance, and comfort, so the method should match the job the edge must do.",miniExample:`Choose ${title} when the project needs ${useFor[0]}; make a small test edge and check it before continuing.`,stepByStep:["Read the pattern and confirm the required stitch count.","Make the cast-on slowly with even, relaxed tension.","Count every stitch before working the first row or joining in the round.","Check the edge for stretch, stability, and twists after the first few stitches or rounds."],commonMistakes:mistakes,troubleshooting:fixes,relatedTools:castOnRelatedTools,relatedProjectTypes:useFor.map(item=>item[0].toUpperCase()+item.slice(1)),relatedEntries:["cast-on-methods"],nextLearningSteps:["Cast-on Methods","Gauge Basics","Reading Written Patterns"],keywords:["cast on","cast-on",title.toLowerCase(),...useFor,...pros,...cons],aliases});
}
function castOnTroubleshootingEntry({id,title,symptoms,causes,fixes,prevention}){
  return wikiEntry({id,title,category:"Troubleshooting Hub",subcategory:"Cast-on Methods",craftTypes:["knitting"],level:"beginner",summary:`Diagnose and fix ${title.toLowerCase()} before working further into the project.`,fullExplanation:`Check the visible symptoms, identify the likely cause, then restart or adjust the edge while it is still quick to correct.`,whenToUse:symptoms.join(" "),whyItMatters:"Cast-on faults affect the first row, finished stretch, fit, and whether work joined in the round can lie correctly.",miniExample:fixes[0],stepByStep:["Stop before working more rows.","Compare the edge with the symptoms in this guide.",...fixes,"Test the corrected edge before continuing."],commonMistakes:causes,troubleshooting:fixes,relatedTools:castOnRelatedTools,relatedProjectTypes:["Socks","Hats","Sweaters","Cuffs","Projects worked in the round"],relatedEntries:["cast-on-methods"],nextLearningSteps:["Cast-on Methods","Long-Tail Cast-On","German Twisted Cast-On / Old Norwegian Cast-On"],keywords:[title.toLowerCase(),...symptoms,...causes],diagnosticFlow:{symptoms,likelyCauses:causes,quickChecks:["Try entering the first stitch without forcing the needle.","Stretch the edge gently and inspect gaps.","Lay all stitches flat if joining in the round."],decisionPath:["Match the visible symptom.","Check tension, method, needle size, tail, and stitch orientation.","Choose the smallest reliable fix; restart when the edge structure is wrong."],fixes,prevention,saveToProjectAction:"add-checklist"}});
}
libraryWikiEntries.push(
  wikiEntry({id:"cast-on-methods",title:"Cast-on Methods",category:"Knitting Knowledge",subcategory:"Cast-on Methods",craftTypes:["knitting"],level:"advanced",difficulty:"Beginner to Advanced",estimatedReadTime:"12 min",summary:"Choose a knitting cast-on by the edge's need for stretch, firmness, decoration, temporary live stitches, or a polished tubular finish.",fullExplanation:"Casting on creates the first live stitches of a knitted project. The edge is part of the finished fabric: its structure affects stretch, firmness, appearance, fit, and how comfortably the first row can be worked. Stretchy methods suit cuffs and openings; firm methods add structure; decorative methods make the edge a feature; provisional methods preserve live stitches; tubular methods flow cleanly into ribbing.",whenToUse:"Use this guide before starting a project, adding stitches mid-row, choosing an edge for ribbing or socks, planning a temporary edge, or diagnosing a cast-on that is tight, loose, uneven, or twisted.",whyItMatters:"The wrong edge can stop a sock fitting over a heel, make a neckline uncomfortable, distort a blanket, or leave an unstable first row even when gauge is correct.",miniExample:"For cuff-down socks choose German Twisted; for toe-up socks choose Judy’s Magic or Turkish; for a polished 1x1 rib choose Tubular; for a firm button band choose Cable.",stepByStep:["Identify the project type and what the edge must do.","Decide whether stretch, firmness, decoration, provisional live stitches, or a tubular finish matters most.","Consider yarn type, needle size, whether the work is flat or in the round, and your skill level.","Choose a method: stretchy—German Twisted, Tubular, or Long-Tail on a larger needle; firm—Cable or Knitted; beginner general use—Long-Tail or Knitted; temporary—Provisional or Crochet Provisional; decorative—Picot or I-Cord; mid-row—Backward Loop, Knitted, or Cable.","Make a small test edge, check stretch and stitch count, and lay stitches flat before joining in the round."],commonMistakes:["Casting on too tightly and making the first row hard to knit.","Using Backward Loop for a long project edge and creating loose gaps.","Underestimating the tail for a long-tail method.","Joining in the round before checking that the edge is not twisted.","Choosing a firm cast-on for a sock, hat, or ribbed opening that needs stretch."],troubleshooting:["Too tight: restart on a larger needle or use German Twisted or Tubular; keep loops relaxed and test stretch.","Too loose: replace a long Backward Loop edge with Long-Tail, Knitted, or Cable and control tension.","Tail ran out: restart more generously or use a two-strand Long-Tail or Knitted cast-on.","Twisted in the round: undo and rejoin early; after several rounds a restart is usually the cleanest fix.","First row is hard to knit: restart with a larger needle, two needles, or a stretchier method."],relatedTools:castOnRelatedTools,relatedProjectTypes:["Socks","Hats","Sweaters","Cuffs","Mittens","Shawls","Blankets","Bags","Buttonholes"],relatedEntries:["backward-loop-cast-on","long-tail-cast-on","knitted-cast-on","cable-cast-on","german-twisted-cast-on","tubular-cast-on","provisional-cast-on","judys-magic-cast-on","turkish-cast-on","figure-8-cast-on","picot-cast-on","i-cord-cast-on","crochet-cast-on"],nextLearningSteps:["Long-Tail Cast-On","German Twisted Cast-On / Old Norwegian Cast-On","Tubular Cast-On","Provisional Cast-On"],keywords:["cast on choice guide","stretchy cast on","firm cast on","decorative cast on","provisional cast on","tubular cast on","cast on too tight","cast on too loose","tail too short","twisted cast on","first row hard to knit"],aliases:["casting on","knitting cast ons","best cast on"]}),
  castOnMethodEntry({id:"backward-loop-cast-on",title:"Backward Loop Cast-On",level:"beginner",useFor:["adding stitches mid-row","quick simple starts","buttonholes"],pros:["very easy","fast"],cons:["loose and unstable","hard to knit into","not ideal for large edges"],mistakes:["Loops are too loose.","The edge becomes uneven.","The first row feels messy."],fixes:["For a project edge, choose a more stable cast-on if loops stay loose.","When adding stitches mid-row, keep the loop tension controlled."]}),
  castOnMethodEntry({id:"long-tail-cast-on",title:"Long-Tail Cast-On",level:"beginner",difficulty:"Beginner / Intermediate",useFor:["scarves","hats","sweaters","general project starts"],pros:["neat","moderately stretchy","strong","common in patterns"],cons:["tail length is hard to estimate","can become too tight"],mistakes:["Not leaving enough tail.","Casting on too tightly.","Twisting stitches before joining in the round."],fixes:["Use a larger needle if the edge is too tight.","Use a two-strand method if tail estimation is difficult.","Lay stitches flat and check they are not twisted before joining."],aliases:["long tail cast on"]}),
  castOnMethodEntry({id:"knitted-cast-on",title:"Knitted Cast-On",level:"beginner",useFor:["beginners","stable edges","adding stitches at the beginning of rows"],pros:["easy to understand","uses the knit-stitch movement","stable"],cons:["less stretchy than some methods"],mistakes:["Pulling too tightly between stitches.","Making an uneven edge."],fixes:["Keep a little space between stitches.","Use a larger needle if the edge is tight."]}),
  castOnMethodEntry({id:"cable-cast-on",title:"Cable Cast-On",level:"intermediate",difficulty:"Beginner / Intermediate",useFor:["firm edges","button bands","edges needing structure"],pros:["neat","firm","stable"],cons:["not very stretchy","can be tight"],mistakes:["Inserting the needle in the wrong place.","Pulling too tightly."],fixes:["Avoid it for sock cuffs or tight hat brims unless the pattern asks for it.","Use a larger needle if needed."]}),
  castOnMethodEntry({id:"german-twisted-cast-on",title:"German Twisted Cast-On / Old Norwegian Cast-On",level:"intermediate",useFor:["socks","hats","cuffs","mittens","stretchy edges"],pros:["stretchy","strong","good for ribbing"],cons:["harder hand movement","uses more tail yarn"],mistakes:["Not leaving enough tail.","Making the twisting motion incorrectly.","Creating an uneven edge."],fixes:["Practise the movement slowly.","Leave extra tail.","Use this when Long-Tail is too tight."],aliases:["old norwegian cast on","german twisted cast on"]}),
  castOnMethodEntry({id:"tubular-cast-on",title:"Tubular Cast-On",level:"advanced",useFor:["1x1 ribbing","sweater hems","cuffs","polished edges"],pros:["professional appearance","stretchy","blends into ribbing"],cons:["complicated","requires setup rows","easy to misalign"],mistakes:["Working the wrong setup row.","Misaligning the ribbing.","Twisting the edge."],fixes:["Use stitch markers.","Check 1x1 rib alignment before continuing.","Choose it for polished garment edges."]}),
  castOnMethodEntry({id:"provisional-cast-on",title:"Provisional Cast-On",level:"advanced",difficulty:"Intermediate / Advanced",useFor:["temporary edges","folded hems","grafting","live stitches needed later","two-direction scarves and shawls"],pros:["removable","creates live stitches","supports advanced construction"],cons:["can be confusing","waste yarn can be hard to remove"],mistakes:["Using fuzzy waste yarn.","Picking up the wrong strand.","Twisting live stitches."],fixes:["Use smooth, contrasting waste yarn.","Take notes or photos before removing it.","Check stitch orientation before knitting."]}),
  castOnMethodEntry({id:"judys-magic-cast-on",title:"Judy’s Magic Cast-On",level:"advanced",difficulty:"Intermediate / Advanced",useFor:["toe-up socks","seamless starts","bags","closed tubes"],pros:["seamless","no visible seam","excellent for sock toes"],cons:["fiddly at first","stitches can twist"],mistakes:["Leaving tension too loose.","Twisting the first round.","Moving the needle in the wrong direction."],fixes:["Tighten gently after the first few rounds.","Check top and bottom stitches before continuing."],aliases:["judy's magic cast on","judys magic cast on"]}),
  castOnMethodEntry({id:"turkish-cast-on",title:"Turkish Cast-On",level:"intermediate",useFor:["toe-up socks","seamless closed starts","small tubes"],pros:["simple once understood","seamless"],cons:["loose first rounds","needs a circular needle or DPNs"],mistakes:["Wrapping too loosely.","Losing stitches."],fixes:["Knit the first rounds slowly.","Use stitch markers if needed."]}),
  castOnMethodEntry({id:"figure-8-cast-on",title:"Figure-8 Cast-On",level:"intermediate",useFor:["toe-up socks","closed starts"],pros:["seamless"],cons:["loose and fiddly"],mistakes:["Uneven tension.","A twisted first round."],fixes:["Tighten carefully after a few rounds.","Try Judy’s Magic Cast-On if it is easier for you."],aliases:["figure 8 cast on"]}),
  castOnMethodEntry({id:"picot-cast-on",title:"Picot Cast-On",level:"intermediate",useFor:["decorative edges","baby items","decorative garment edges","shawls"],pros:["decorative","pretty edge"],cons:["not always stretchy","slower"],mistakes:["Making uneven picots.","Pulling the edge too tight."],fixes:["Count repeats carefully.","Use markers for repeat sections."]}),
  castOnMethodEntry({id:"i-cord-cast-on",title:"I-Cord Cast-On",level:"advanced",useFor:["neat rounded edges","shawls","blankets","bags","decorative garment edges"],pros:["polished","structured","decorative"],cons:["slow","uses more yarn"],mistakes:["Uneven cord tension.","Letting the edge pull in."],fixes:["Keep the cord relaxed.","Check fabric width after a few rows."],aliases:["icord cast on"]}),
  castOnMethodEntry({id:"crochet-cast-on",title:"Crochet Cast-On",level:"intermediate",useFor:["provisional cast-ons","neat chain-like edges","flexible starts"],pros:["tidy","familiar to crocheters"],cons:["can be confused with the provisional version"],mistakes:["Crocheting too tightly.","Picking up the wrong part of the chain."],fixes:["Use a larger hook if the chain is tight.","Use contrasting waste yarn for the provisional version."],aliases:["crochet provisional cast on"]}),
  castOnTroubleshootingEntry({id:"cast-on-too-tight",title:"My Cast-On Is Too Tight",symptoms:["The first row is hard to knit.","The edge does not stretch.","A sock or hat cuff feels tight."],causes:["Cast-on tension is too tight.","The method is too firm for the edge.","The needle is too small."],fixes:["Restart with a larger needle.","Use German Twisted or Tubular for a stretchy edge.","Keep the loops relaxed."],prevention:["Test edge stretch before continuing."]}),
  castOnTroubleshootingEntry({id:"cast-on-too-loose",title:"My Cast-On Is Too Loose",symptoms:["The edge looks messy.","The first row has large gaps.","The fabric starts unevenly."],causes:["Backward Loop was used for too many stitches.","Cast-on tension is too loose."],fixes:["Use Long-Tail, Knitted, or Cable Cast-On.","Tighten gently after the first row."],prevention:["Choose a stable cast-on for a long project edge."]}),
  castOnTroubleshootingEntry({id:"cast-on-tail-too-short",title:"I Did Not Leave Enough Tail",symptoms:["The yarn tail runs out before all stitches are cast on."],causes:["The tail was estimated too short."],fixes:["Restart the cast-on.","Use a two-strand Long-Tail Cast-On.","Estimate more generously."],prevention:["Leave extra tail or use Knitted Cast-On instead."]}),
  castOnTroubleshootingEntry({id:"cast-on-twisted-in-round",title:"My Cast-On Twists When Joining in the Round",symptoms:["The round is twisted after joining."],causes:["The cast-on edge was not laid flat before joining."],fixes:["If only the first round is worked, undo and rejoin.","If several rounds are worked, restarting is usually the cleanest fix."],prevention:["Lay stitches flat and check the cast-on edge faces inward all the way around."]}),
  castOnTroubleshootingEntry({id:"cast-on-first-row-hard",title:"My First Row Is Hard to Knit",symptoms:["Stitches are too tight on the needle.","The needle cannot enter easily."],causes:["The cast-on is too tight.","The needle size is too small for the edge."],fixes:["Restart with a larger needle.","Use a stretchier method."],prevention:["Cast onto two needles or a larger needle when appropriate."]})
);
...
function libraryEntryBadgeHtml(entry){
  ...
  const level=entry.difficulty&&entry.difficulty!==entry.level?entry.difficulty:(libraryLevelLabels[entry.level]||entry.level);
  return `<div class="wiki-badges"><span>${escapeHtml(level)}</span>${craft}<span>${escapeHtml(entry.sourceQuality||libraryReviewStatusLabel(entry.verifiedStatus))}</span><span>${escapeHtml(entry.estimatedReadTime)}</span></div>`;
}
...
function openYarnMaterialModal(materialId=null){
  ...
  openModal(`<p class="eyebrow">YARN MATERIAL</p><h2>${material?"Edit":"Add"} yarn material</h2><div class="form-grid"><div class="field full"><label for="material-name">Material name</label><input id="material-name" value="${escapeHtml(material?.name||"")}"></div><div class="field"><label for="material-category">Fibre group</label><select id="material-category">${["Natural","Synthetic","Blends","Other"].map(v=>`<option ${material?.category===v?"selected":""}>${v}</option>`).join("")}</select></div><div class="field"><label for="material-other">Specify Other</label><input id="material-other" value="${escapeHtml(material?.otherCategory||"")}" placeholder="Required for Other"></div><div class="field full upload-drop"><label for="material-photo">Material photo</label><input id="material-photo" type="file" accept="image/*"><small>${material?.imageAsset?"Select a new photo to replace the current one.":"Optional reference photo."}</small></div><div class="field full"><label for="material-season">Best season</label><input id="material-season" value="${escapeHtml(material?.season||"")}" placeholder="e.g. warm weather, winter, all season"></div><div class="field full"><label>Texture</label><div class="texture-checks">${textures.map(t=>`<label class="check-row"><input type="checkbox" name="material-texture" value="${t}" ${(material?.textures||[]).includes(t)?"checked":""}><span>${t}</span></label>`).join("")}</div></div><div class="field full"><label for="material-notes">Useful to know</label><textarea id="material-notes" rows="5">${escapeHtml(material?.features||"")}</textarea></div></div>${material?`<section class="danger-zone compact-danger-zone"><strong>Danger Zone</strong><p>Delete this yarn material only if you no longer need it. This action cannot be undone.</p><button type="button" class="danger-button secondary-button" id="delete-material-inside-edit">Delete item</button></section>`:""}<div class="modal-actions"><button class="secondary-button" onclick="closeModal()">Cancel</button><button class="primary-button" id="save-material">Save material</button></div>`);
...
function renderSettings(){
  ...
  host.innerHTML=`<div class="page-title"><p class="eyebrow">YOUR YARNCHA</p><h1>Settings</h1><p>Personalise your workspace and keep your projects safely within reach.</p></div>
    <div class="settings-page-shell">
      ...
      <section class="card mobile-card settings-panel settings-panel-wide appearance-panel" id="settings-appearance" tabindex="-1"><div class="settings-section-heading"><span class="settings-section-icon">${uiIcon("appearance","ui-icon")}</span><div><p class="eyebrow">APPEARANCE</p><h2>Make Yarncha feel like yours</h2><p>Choose a colour mood, design style, display mode, and language.</p></div></div>
  ...
  document.getElementById("settings-phone-help").onclick=openPhoneHelp;
  ...
  panel.innerHTML=`<div class="tools-page-shell">
    ...
    <section class="toolbox-browser card">
      <div class="toolbox-controls">
        <label for="tool-search">Search tools</label><input id="tool-search" class="toolbox-search" value="${escapeHtml(currentToolSearch)}" placeholder="Search tools, e.g. gauge, sock, yarn, C2C">
      </div>
...
function converterHtml(active) {
  ...
  if (active === "length") {
    ...
    body = `<div class="form-grid"><div class="field"><label for="convert-input">Amount</label><input id="convert-input" type="number" value="100"></div><div class="field"><label for="convert-from">From</label><select id="convert-from">${units.map(u=>`<option>${u}</option>`).join("")}</select></div></div><div id="conversion-result"></div>`;
    ...
    body=`<div class="form-grid"><div class="field"><label for="yarn-grams">Ball weight (grams)</label><input id="yarn-grams" type="number" min="1" placeholder="e.g. 100"></div><div class="field"><label for="yarn-yards">Length (yards)</label><input id="yarn-yards" type="number" min="1" placeholder="e.g. 220"></div><div class="field"><label for="yarn-mm">Yarn thickness estimate (mm)</label><input id="yarn-mm" type="number" min=".1" step=".1" placeholder="e.g. 1.8"></div><div class="field"><label for="yarn-wpi">Wraps per inch (WPI)</label><input id="yarn-wpi" type="number" min="1" placeholder="e.g. 12"></div><div class="field full"><label for="yarn-known-weight">Known yarn weight (optional)</label><select id="yarn-known-weight"><option value="-1">Determine it for me</option>${yarnWeights.map((r,i)=>`<option value="${i}">${r.name} · CYC ${r.cyc}</option>`).join("")}</select></div></div><button class="secondary-button" id="classify-yarn">Suggest category and tools</button><div id="yarn-class-result"></div><div style="overflow:auto"><table class="conversion-table"><thead><tr>${["Category","CYC","Australia/NZ","UK/EU name","Chinese","WPI","Needles","Hooks"].map(h=>`<th>${h}</th>`).join("")}</tr></thead><tbody>${yarnWeights.map(r=>`<tr>${[r.name,r.cyc,r.au,r.uk,r.cn,r.wpi,r.needles,r.hooks].map(c=>`<td>${c}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`;
  }
...
function toolSizeAssistantHtml(){return `<div class="size-assistant card"><p class="eyebrow">BUILT-IN SIZE ASSISTANT</p><h3>Which hook or needle should I try?</h3><div class="form-grid"><div class="field"><label for="size-craft">Craft</label><select id="size-craft"><option value="crochet">Crochet</option><option value="knitting">Knitting</option><option value="tunisian">Tunisian crochet</option></select></div><div class="field"><label for="size-yarn">Yarn category</label><select id="size-yarn">${yarnWeights.map((r,i)=>`<option value="${i}">${r.name} · CYC ${r.cyc}</option>`).join("")}</select></div><div class="field full"><label for="size-goal">Fabric goal</label><select id="size-goal"><option value="balanced">Balanced / pattern starting point</option><option value="tight">Tighter and denser</option><option value="drapey">Looser and more drapey</option></select></div></div><button class="secondary-button" id="suggest-tool-size">Suggest a starting size</button><div id="size-assistant-result"></div></div>`;}
...
function openModal(html,options={}) {
  ...
  content.innerHTML=String(html).replaceAll('onclick="closeModal()"','data-close-modal');
  content.querySelectorAll("[data-close-modal]").forEach(button=>button.addEventListener("click",()=>closeModal()));
  ...
  modal.dataset.size=content.querySelector(".account-menu-sheet")?"menu":content.querySelector(".symbol-detail,.symbol-edit-form,.ocr-review-layout,.chart-cell-editor,.sync-compare-grid")?"wide":"normal";
  modal.setAttribute("aria-label",options.label||content.querySelector("h2")?.textContent||"Dialog");
  document.getElementById("modal-backdrop").classList.add("open");
  if(modalLastFocus?.id==="account-button")modalLastFocus.setAttribute("aria-expanded","true");
  const firstFocus=content.querySelector("input:not([type=hidden]),select,textarea,button");
  firstFocus?.focus({preventScroll:true});
  requestAnimationFrame(()=>{if(!modal.contains(document.activeElement))firstFocus?.focus({preventScroll:true});});
}
function closeModal(force=false) {
  ...
  if(target?.id==="account-button")target.setAttribute("aria-expanded","false");
  if(target?.isConnected)target.focus?.({preventScroll:true});
  requestAnimationFrame(()=>{if(target?.isConnected&&document.activeElement!==target)target.focus?.({preventScroll:true});});
  return true;
}
...
async function importBackup(event,mode="merge"){
  try{
    ...
    const security=window.YarnchaBackupSecurity;
    security.assertBackupFileSize(file.size);
    const parsed=security.validateBackupDocument(JSON.parse(await file.text()),file.size);
    for(const [id,asset] of Object.entries(parsed.assets)){
      const blob=await dataUrlToBlob(asset.data);
      const restored=new File([blob],asset.name,{type:asset.type,lastModified:asset.lastModified});
    ...
    if(mode==="replace"){
      ...
      const projects=(imported.projects||[]).map((p,index)=>normalizeProjectRecord(existingIds.has(p.id)?{...p,id:`project-imported-${Date.now()}-${index}`}:{...p},index));
    ...
    console.warn("[Yarncha backup] Import rejected",String(error?.message||error));
    toast("This backup was rejected because it is invalid or unsafe.");
...
function openProjectModal() {
  openModal(`<p class="eyebrow">CHART FIRST</p><h2>New project</h2><div class="form-grid">
    <div class="field full"><label for="new-name">Project name</label><input id="new-name" placeholder="e.g. Summer lace scarf"></div>
    <div class="field full"><label for="new-type">Craft</label><select id="new-type">${projectTypeOptions.map(t=>`<option>${t}</option>`).join("")}</select></div>
    <div class="field full upload-drop"><label for="new-chart">Pattern charts (multiple PDF, PNG or JPG)</label><input id="new-chart" type="file" accept=".pdf,image/*" multiple><small>Up to 30 PDF pages are indexed per file. You can add more files later.</small></div></div>
...
function openSubCounterModal(editId=null){
  const existing=editId?getProject().subCounters.find(item=>item.id===editId):null,counter=normalizeSubCounter(existing||{name:"",mode:"repeatCounter",linked:true,start:0,count:0,every:1,step:1,resetValue:0,color:"#718c72",anchorRow:Number(getProject().row)||0,sectionStartProjectPosition:Number(getProject().row)||0,localStartValue:0});
  const projectRow=Number(getProject().row)||0;
  let mode=counter.mode==="subCounter"?"subCounter":"repeatCounter";
  const drafts={repeatCounter:{name:existing?.name||"",notes:counter.notes||"",firstTrigger:String(counter.startAt??counter.anchorRow??0),interval:String(counter.repeatValue||counter.every||1),unit:counter.unit==="round"?"round":"row",endAt:counter.endAt??"",linked:counter.linked!==false},subCounter:{name:existing?.name||counter.sectionName||"",sectionStart:String(counter.sectionStartProjectPosition??counter.anchorRow??projectRow),startValue:String(counter.localStartValue??counter.start??0),step:String(counter.step||1),linked:counter.linked!==false,color:counter.color||"#718c72"}};
  openModal(`<section class="repeat-engine-modal"><p class="eyebrow">REPEAT ENGINE</p><h2>${existing?"Edit repeat rule":"Add repeat rule"}</h2><p class="muted-copy">Choose one counter type. Only settings for that type appear below.</p><div class="repeat-mode-toggle" role="group" aria-label="Repeat mode"><label><input type="radio" name="repeat-mode" value="repeatCounter" ${mode==="repeatCounter"?"checked":""}><span>Repeat Counter</span></label><label><input type="radio" name="repeat-mode" value="subCounter" ${mode==="subCounter"?"checked":""}><span>Sub-Counter / New Section</span></label></div><div id="repeat-mode-fields"></div><p class="form-error" id="repeat-rule-error" role="alert"></p><div class="modal-actions repeat-modal-actions"><button class="secondary-button" onclick="closeModal()">Cancel</button>${existing?`<button class="secondary-button" id="disable-repeat-rule">${counter.enabled===false?"Enable":"Disable"}</button>`:""}<button class="primary-button" id="create-sub"></button></div></section>`);
  const capture=()=>{if(mode==="repeatCounter")drafts.repeatCounter={name:valueOf("repeat-name"),notes:valueOf("repeat-notes"),firstTrigger:valueOf("repeat-first-trigger"),interval:valueOf("repeat-interval"),unit:valueOf("repeat-unit"),endAt:valueOf("repeat-end"),linked:valueOf("repeat-linked")!=="no"};else drafts.subCounter={name:valueOf("section-name"),sectionStart:valueOf("section-start"),startValue:valueOf("section-start-value"),step:valueOf("section-step"),linked:valueOf("section-linked")!=="no",color:valueOf("section-color")||"#718c72"};};
  const repeatDraft=()=>{const d=drafts.repeatCounter;return {id:counter.repeatRuleId||counter.repeatRule?.id,mode:"repeatCounter",repeatType:d.unit==="round"?"every-x-rounds":"every-x-rows",repeatValue:d.interval,unit:d.unit,firstTrigger:d.firstTrigger,startAt:d.firstTrigger,endAt:d.endAt,enabled:counter.enabled!==false,notes:d.notes,linkedFeature:"project"};};
  const updatePreview=()=>{capture();const error=document.getElementById("repeat-rule-error");error.textContent="";if(mode!=="repeatCounter")return;const validation=repeatEngine().validateRepeatRule(repeatDraft()),preview=document.getElementById("repeat-rule-preview");if(!validation.valid){error.textContent=validation.errors[0];preview.innerHTML="";return;}const rule=validation.rule,triggers=repeatEngine().getTriggerPositions(rule,{from:rule.startAt,limit:5});preview.innerHTML=`<p class="repeat-preview-summary">First trigger is inclusive</p><div class="repeat-preview-pills">${triggers.map(trigger=>`<span class="repeat-preview-chip">${trigger}</span>`).join("")}</div><p class="repeat-preview-next">${triggers.join(" · ")}</p>`;};
  const renderFields=()=>{const host=document.getElementById("repeat-mode-fields"),d=drafts[mode];host.innerHTML=mode==="repeatCounter"?`<div class="repeat-engine-form"><section class="repeat-form-section"><h3>Rule</h3><div class="field"><label for="repeat-name">Rule name</label><input id="repeat-name" value="${escapeHtml(d.name)}" placeholder="Decrease every 8 rows"></div><div class="field"><label for="repeat-notes">Optional action or instruction</label><input id="repeat-notes" value="${escapeHtml(d.notes)}" placeholder="Increase, decrease, pattern row, or custom note"></div></section><section class="repeat-form-section"><h3>Timing</h3><div class="field"><label for="repeat-first-trigger">First trigger row / round</label><input id="repeat-first-trigger" type="number" min="0" step="1" value="${escapeHtml(d.firstTrigger)}"><small>This exact row is the first time the rule happens.</small></div><div class="field"><label for="repeat-interval">Repeat every</label><input id="repeat-interval" type="number" min="1" step="1" value="${escapeHtml(d.interval)}"></div><div class="field"><label for="repeat-unit">Unit</label><select id="repeat-unit"><option value="row" ${d.unit!=="round"?"selected":""}>Rows</option><option value="round" ${d.unit==="round"?"selected":""}>Rounds</option></select></div><div class="field"><label for="repeat-end">Optional stop condition</label><input id="repeat-end" type="number" min="0" step="1" value="${escapeHtml(d.endAt)}" placeholder="No stop row"></div></section><section class="repeat-form-section"><h3>Behaviour</h3><div class="field"><label for="repeat-linked">Link to Main Row Counter</label><select id="repeat-linked"><option value="yes" ${d.linked?"selected":""}>On</option><option value="no" ${!d.linked?"selected":""}>Off</option></select></div></section><section class="repeat-form-section repeat-preview-card"><h3>Trigger preview</h3><div id="repeat-rule-preview" aria-live="polite"></div></section></div>`:`<div class="repeat-engine-form"><section class="repeat-form-section"><h3>Counter details</h3><div class="field"><label for="section-name">Counter name</label><input id="section-name" value="${escapeHtml(d.name)}" placeholder="Sleeve shaping"></div></section><section class="repeat-form-section"><h3>Starting point</h3><div class="field"><label for="section-start">Section begins at main row / round</label><input id="section-start" type="number" min="0" step="1" value="${escapeHtml(d.sectionStart)}"><small>The main counter position where this separate section starts.</small></div><div class="field"><label for="section-start-value">Sub-counter starts at</label><input id="section-start-value" type="number" step="1" value="${escapeHtml(d.startValue)}"><small>The first number shown on this sub-counter.</small></div></section><section class="repeat-form-section"><h3>Counter behaviour</h3><div class="field"><label for="section-step">Increment step</label><input id="section-step" type="number" step="1" value="${escapeHtml(d.step)}"></div><div class="field"><label for="section-linked">Link to Main Row Counter</label><select id="section-linked"><option value="yes" ${d.linked?"selected":""}>On</option><option value="no" ${!d.linked?"selected":""}>Off</option></select></div><div class="field"><label for="section-color">Optional colour tag</label><input id="section-color" type="color" value="${escapeHtml(d.color)}"></div></section></div>`;document.getElementById("create-sub").innerHTML=mode==="repeatCounter"?'<span class="repeat-save-desktop">Save Repeat Rule</span><span class="repeat-save-mobile">Save rule</span>':'<span class="repeat-save-desktop">Save Sub-Counter</span><span class="repeat-save-mobile">Save counter</span>';host.querySelectorAll("input,select,textarea").forEach(element=>element.addEventListener("input",updatePreview));updatePreview();};
  document.querySelectorAll('[name="repeat-mode"]').forEach(element=>element.addEventListener("change",event=>{capture();mode=event.target.value;document.getElementById("repeat-rule-error").textContent="";renderFields();}));
  renderFields();
  document.getElementById("create-sub").onclick=()=>{capture();const error=document.getElementById("repeat-rule-error");let values;if(mode==="repeatCounter"){const d=drafts.repeatCounter;if(!d.name.trim()){error.textContent="Add a rule name.";return;}const validation=repeatEngine().validateRepeatRule(repeatDraft());if(!validation.valid){error.textContent=validation.errors[0];return;}values=normalizeSubCounter({id:existing?.id,name:d.name.trim(),mode,linked:d.linked,every:validation.rule.repeatValue,repeatValue:validation.rule.repeatValue,repeatType:validation.rule.repeatType,unit:validation.rule.unit,startAt:validation.rule.startAt,anchorRow:validation.rule.startAt,endAt:validation.rule.endAt,repeatRule:validation.rule,repeatRuleId:validation.rule.id,start:0,count:existing?.count??0,step:1,resetValue:0,color:existing?.color||"#718c72",notes:d.notes,lastVoiceRow:existing?.lastVoiceRow,enabled:existing?.enabled!==false});}else{const d=drafts.subCounter,start=Number(d.sectionStart),startValue=Number(d.startValue),step=Number(d.step);if(!d.name.trim()){error.textContent="Add a counter name.";return;}if(!Number.isFinite(start)||start<0){error.textContent="Section start must be a valid non-negative number.";return;}if(!Number.isFinite(startValue)){error.textContent="Sub-counter starting value must be a valid number.";return;}if(!Number.isFinite(step)||step===0){error.textContent="Increment step must be a non-zero number.";return;}const rule=repeatEngine().createRepeatRule({id:counter.repeatRuleId||counter.repeatRule?.id,mode,sectionName:d.name.trim(),repeatType:"every-row",repeatValue:1,unit:"row",startAt:startValue,localStartValue:startValue,sectionStartProjectPosition:start,enabled:counter.enabled!==false});values=normalizeSubCounter({id:existing?.id,name:d.name.trim(),mode,sectionName:d.name.trim(),linked:d.linked,every:1,repeatValue:1,repeatType:"every-row",unit:"row",startAt:startValue,anchorRow:start,sectionStartProjectPosition:start,localStartValue:startValue,repeatRule:rule,repeatRuleId:rule.id,start:startValue,count:existing?.count??startValue,step,resetValue:startValue,color:d.color,lastVoiceRow:existing?.lastVoiceRow,enabled:existing?.enabled!==false});}values.syncRow=projectRow;values.syncCount=values.count;if(existing)Object.assign(existing,values);else getProject().subCounters.push(values);getProject().repeatRules=repeatEngine().migrateRepeatRules([getProject()])[0].repeatRules;saveProjectTouch(getProject());closeModal();renderProjectDetail();toast(existing?"Counter updated":"Counter added");};
  document.getElementById("disable-repeat-rule")?.addEventListener("click",()=>{if(!existing)return;existing.enabled=existing.enabled===false;existing.repeatRule=repeatEngine().createRepeatRule({...existing.repeatRule,enabled:existing.enabled});getProject().repeatRules=repeatEngine().migrateRepeatRules([getProject()])[0].repeatRules;saveProjectTouch(getProject());closeModal();renderProjectDetail();toast(existing.enabled?"Repeat rule enabled":"Repeat rule disabled");});
}
function openSubCounterModalLegacy(editId=null) {
  ...
  openModal(`<section class="repeat-engine-modal">
    ...
    <div class="repeat-engine-grid">
      <div class="repeat-settings-card">
        ...
        <div class="form-grid">
          <div class="field full"><label for="sub-name">Counter name</label><input id="sub-name" value="${escapeHtml(existing?counter.name:"")}" placeholder="e.g. Sleeve Increase"></div>
          <div class="field"><label for="sub-repeat-type">Repeat type</label><select id="sub-repeat-type">${typeOptions.map(([value,label])=>`<option value="${value}" ${counter.repeatType===value?"selected":""}>${label}</option>`).join("")}</select></div>
          <div class="field"><label for="sub-every">Repeat every / Repeat value</label><input id="sub-every" type="number" min="1" value="${counter.repeatValue||counter.every}"></div>
          <div class="field"><label for="sub-unit">Unit</label><select id="sub-unit"><option value="row" ${counter.unit!=="round"?"selected":""}>Rows</option><option value="round" ${counter.unit==="round"?"selected":""}>Rounds</option></select></div>
          <div class="field"><label for="sub-start-at">Start row / round</label><input id="sub-start-at" type="number" min="0" value="${counter.startAt??counter.start}"></div>
          <div class="field"><label for="sub-count">Current count</label><input id="sub-count" type="number" min="0" value="${counter.count}"></div>
          <div class="field"><label for="sub-start">Starting value</label><input id="sub-start" type="number" min="0" value="${counter.start}"></div>
          <div class="field"><label for="sub-step">Increment step</label><input id="sub-step" type="number" min="1" value="${counter.step}"></div>
          <div class="field"><label for="sub-linked">Link to Main Row Counter</label><select id="sub-linked"><option value="yes" ${counter.linked!==false?"selected":""}>On</option><option value="no" ${counter.linked===false?"selected":""}>Off</option></select></div>
      ...
      <div class="repeat-settings-card sub-counter-settings-card" id="sub-section-card">
        ...
        <div class="form-grid">
          <div class="field full"><label for="sub-section-name">Section name</label><input id="sub-section-name" value="${escapeHtml(counter.sectionName||counter.name||"")}" placeholder="Sleeve, Neckline, Border, Pattern Repeat"></div>
          <div class="field"><label for="sub-section-start">Section starts at project row / round</label><input id="sub-section-start" type="number" min="0" value="${sectionStartValue}"></div>
          <div class="field"><label for="sub-local-start">Local counter starts from</label><select id="sub-local-start"><option value="0" ${Number(counter.localStartValue)!==1?"selected":""}>0</option><option value="1" ${Number(counter.localStartValue)===1?"selected":""}>1</option></select></div>
          <div class="field"><label for="sub-anchor">Anchor Row</label><input id="sub-anchor" type="number" min="0" value="${counter.anchorRow}" placeholder="Start counting from row"></div>
        </div>
      ...
      <details class="repeat-settings-card repeat-advanced-settings">
        ...
        <div class="form-grid">
          <div class="field"><label for="sub-end-at">End row / round</label><input id="sub-end-at" type="number" min="0" value="${counter.endAt??""}" placeholder="No end"></div>
          <div class="field"><label for="sub-repeat-count">Repeat count</label><input id="sub-repeat-count" type="number" min="1" value="${counter.repeatCount??""}" placeholder="Unlimited"></div>
          <div class="field"><label for="sub-offset">Offset</label><input id="sub-offset" type="number" value="${counter.offset||0}"></div>
          <div class="field"><label for="sub-row-side">RS / WS only</label><select id="sub-row-side"><option value="all" ${counter.rowSide==="all"?"selected":""}>All rows</option><option value="RS" ${counter.rowSide==="RS"?"selected":""}>RS only</option><option value="WS" ${counter.rowSide==="WS"?"selected":""}>WS only</option></select></div>
          ...
          <div class="field"><label for="sub-max">Maximum value optional</label><input id="sub-max" type="number" min="0" value="${counter.max??""}" placeholder="No maximum"></div>
          <div class="field"><label for="sub-reset">Reset value</label><input id="sub-reset" type="number" min="0" value="${counter.resetValue}"></div>
          <div class="field"><label for="sub-voice-every">Speak reminder every X rows</label><input id="sub-voice-every" type="number" min="0" value="${counter.voiceEvery}" placeholder="0 = off"></div>
          <div class="field full"><label for="sub-voice-message">Voice reminder message</label><input id="sub-voice-message" value="${escapeHtml(counter.voiceMessage)}" placeholder="e.g. Cable twist now."></div>
          <div class="field full"><label for="sub-notes">Notes</label><textarea id="sub-notes" rows="3" placeholder="Repeat notes, shaping instruction, chart hint...">${escapeHtml(counter.notes)}</textarea></div>
        </div>
      </details>
    </div>
...
function openRowReminderModal(editId=null){
  ...
  openModal(`<p class="eyebrow">ROW REMINDER</p><h2>${existing?"Edit":"Add"} reminder</h2><p class="muted-copy">Yarncha checks reminders whenever the current row changes.</p><div class="form-grid">
    <div class="field full"><label for="reminder-name">Reminder name</label><input id="reminder-name" value="${escapeHtml(existing?r.name:"")}" placeholder="e.g. Sleeve increases"></div>
    <div class="field"><label for="reminder-every">Trigger every X rows/rounds</label><input id="reminder-every" type="number" min="1" value="${r.every}"></div>
    <div class="field"><label for="reminder-start">Start row/round</label><input id="reminder-start" type="number" min="0" value="${r.startRow}"></div>
    <div class="field"><label for="reminder-end">End row/round optional</label><input id="reminder-end" type="number" min="0" value="${r.endRow??""}" placeholder="No end"></div>
    <div class="field full"><label for="reminder-message">Custom reminder message</label><textarea id="reminder-message" rows="3" placeholder="e.g. Upcoming row: increase at both ends.">${escapeHtml(r.message)}</textarea></div>
...
async function deleteProjectAndAssets(p){
  const queuedDeletion=window.YarnchaCloud?.queueProjectDeletion?.(p.id)||{status:"local-only"};
  await Promise.allSettled(assetIdsForProject(p).map(deleteAsset));
  state.projects=state.projects.filter(x=>x.id!==p.id);
  state.activeProjectId=state.projects[0]?.id||null;
  currentProjectId=state.activeProjectId;
  await saveState();
  const syncResult=window.YarnchaCloud?.status?.user?await window.YarnchaCloud.syncNow("project-delete"):queuedDeletion;
  closeModal(true);showView("projects");
  const status=syncResult?.status;
  toast(status==="synced"?"Project deleted everywhere":status==="failed"?"Project deleted here; cloud deletion will retry":"Project deleted here; cloud deletion queued");
}
...
function openCartItemModal(itemId=null){
  ...
  openModal(`<p class="eyebrow">DECISION CART</p><h2>${item?"Adjust":"Add"} shopping item</h2><div class="form-grid">
    <div class="field full"><label for="cart-name">Item</label><input id="cart-name" value="${escapeHtml(item?.name||"")}"></div>
    <div class="field full upload-drop"><label for="cart-image">Item photo</label><input id="cart-image" type="file" accept="image/*"><small>${item?.imageAsset?"A photo is currently attached. Select another to replace it.":"The photo will appear beside the item name."}</small></div>
    <div class="field"><label for="cart-category">Category</label><select id="cart-category">${["Yarn","Tools","DIY kit","Other"].map(c=>`<option ${c===item?.category?"selected":""}>${c}</option>`).join("")}</select></div>
    <div class="field"><label for="cart-quantity">Quantity</label><input id="cart-quantity" type="number" min="1" value="${item?.quantity||1}"></div>
    <div class="field"><label for="cart-currency">Currency</label><select id="cart-currency">${["AUD","HKD","USD","CNY","JPY","EUR"].map(c=>`<option ${c===(item?.currency||"AUD")?"selected":""}>${c}</option>`).join("")}</select><small>Indicative reference rates, ${fxDate}.</small></div>
    <div class="field"><label for="cart-price">Price each</label><input id="cart-price" type="number" min="0" step=".01" value="${item?.price||0}"></div>
    <div class="field"><label for="cart-project">Link to project</label><select id="cart-project"><option value="">No project</option>${state.projects.map(p=>`<option value="${p.id}" ${p.id===item?.projectId?"selected":""}>${escapeHtml(p.name)}</option>`).join("")}</select></div>
    <div class="field full"><label for="cart-reason">Why are you considering it?</label><input id="cart-reason" value="${escapeHtml(item?.reason||"")}" placeholder="Be specific: shortage, replacement, exact project requirement..."></div>
...
document.addEventListener("keydown",event=>{
  ...
  const standardOpen=modalBackdrop?.classList.contains("open");
  const backdrop=standardOpen?modalBackdrop:document.querySelector(".symbol-detail-overlay");
  if(!backdrop)return;
  if(event.key==="Escape"){event.preventDefault();if(standardOpen)closeModal();else closeSymbolDetail();return;}
  if(event.key!=="Tab")return;
  const focusable=[...backdrop.querySelectorAll('button:not([disabled]),input:not([disabled]):not([type="hidden"]),select:not([disabled]),textarea:not([disabled]),a[href],summary,[tabindex="0"]')].filter(node=>!node.hidden&&node.offsetParent!==null);
  if(!focusable.length)return;
  const first=focusable[0],last=focusable.at(-1);
  if(!backdrop.contains(document.activeElement)){event.preventDefault();first.focus();}
...
window.__yarnchaBeforeUnloadSaveGuard=event=>{
  if(saveDirty&&hasLoadedSavedState){
    try{
      state.lastSavedAt=new Date().toISOString();
      localStorage.setItem(STORAGE_KEY,JSON.stringify(state));
      saveDirty=false;
    }catch(error){lastSaveFailed=true;}
  }
...
document.getElementById("mobile-more").onclick=openMobileMoreMenu;
...
window.YarnchaLocal={
  ...
  saveState,
  replaceState(next,{allowEmpty=false}={}){
    if(!allowEmpty&&!next?.projects?.length&&state.projects?.length)return updateSaveStatus("Sync conflict — kept newer device progress");
...
async function configureServiceWorker(){
  ...
  registration.addEventListener("updatefound",()=>{
    ...
    worker?.addEventListener("statechange",()=>{
      if(worker.state==="installed"&&navigator.serviceWorker.controller)updateSaveStatus("A Yarncha update is ready for your next visit");
    });
  });
