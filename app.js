...
function symbolDatabaseHtml(){
  ...
  if(currentSymbolId){
    ...
    return `<section class="symbol-detail card">
      ...
      <div class="symbol-detail-hero"><div class="symbol-glyph">${symbolVisualHtml(entry,"detail")}</div><div><p class="eyebrow">${escapeHtml(entry.craft)} · ${escapeHtml(entry.referenceCategory||entry.category)} · ${escapeHtml(entry.difficulty)}</p><h2>${escapeHtml(entry.nameEn)}</h2><div class="symbol-abbreviation-block" aria-label="Abbreviations"><h3>Abbreviations</h3><div class="symbol-abbreviation-chips ${["Crochet","Tunisian"].includes(entry.craft)?"crochet-abbreviation-chips":""}">${["Crochet","Tunisian"].includes(entry.craft)?`<span><small>US</small><strong>${escapeHtml(abbreviations.us||entry.abbreviationUS||"—")}</strong></span><span><small>UK</small><strong>${escapeHtml(abbreviations.uk||entry.abbreviationUK||"—")}</strong></span>`:`<span><small>UK/US</small><strong>${escapeHtml(abbreviations.usUk||"—")}</strong></span>`}<span><small>CN</small><strong>${escapeHtml(abbreviations.cn||"—")}</strong></span><span><small>JP</small><strong>${escapeHtml(abbreviations.jp||"—")}</strong></span></div></div></div></div>
