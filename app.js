...
function openSubCounterModal(editId=null){
  ...
  const renderFields=()=>{const host=document.getElementById("repeat-mode-fields"),d=drafts[mode];host.innerHTML=mode==="repeatCounter"?`<div class="repeat-engine-form"><section class="repeat-form-section"><h3>Rule</h3><div class="field"><label>Rule name</label><input id="repeat-name" value="${escapeHtml(d.name)}" placeholder="Decrease every 8 rows"></div><div class="field"><label>Optional action or instruction</label><input id="repeat-notes" value="${escapeHtml(d.notes)}" placeholder="Increase, decrease, pattern row, or custom note"></div></section><section class="repeat-form-section"><h3>Timing</h3><div class="field"><label>First trigger row / round</label><input id="repeat-first-trigger" type="number" min="0" step="1" value="${escapeHtml(d.firstTrigger)}"><small>This exact row is the first time the rule happens.</small></div><div class="field"><label>Repeat every</label><input id="repeat-interval" type="number" min="1" step="1" value="${escapeHtml(d.interval)}"></div><div class="field"><label>Unit</label><select id="repeat-unit"><option value="row" ${d.unit!=="round"?"selected":""}>Rows</option><option value="round" ${d.unit==="round"?"selected":""}>Rounds</option></select></div><div class="field"><label>Optional stop condition</label><input id="repeat-end" type="number" min="0" step="1" value="${escapeHtml(d.endAt)}" placeholder="No stop row"></div></section><section class="repeat-form-section"><h3>Behaviour</h3><div class="field"><label>Link to Main Row Counter</label><select id="repeat-linked"><option value="yes" ${d.linked?"selected":""}>On</option><option value="no" ${!d.linked?"selected":""}>Off</option></select></div></section><section class="repeat-form-section repeat-preview-card"><h3>Trigger preview</h3><div id="repeat-rule-preview" aria-live="polite"></div></section></div>`:`<div class="repeat-engine-form"><section class="repeat-form-section"><h3>Counter details</h3><div class="field"><label>Counter name</label><input id="section-name" value="${escapeHtml(d.name)}" placeholder="Sleeve shaping"></div></section><section class="repeat-form-section"><h3>Starting point</h3><div class="field"><label>Section begins at main row / round</label><input id="section-start" type="number" min="0" step="1" value="${escapeHtml(d.sectionStart)}"><small>The main counter position where this separate section starts.</small></div><div class="field"><label>Sub-counter starts at</label><input id="section-start-value" type="number" step="1" value="${escapeHtml(d.startValue)}"><small>The first number shown on this sub-counter.</small></div></section><section class="repeat-form-section"><h3>Counter behaviour</h3><div class="field"><label>Increment step</label><input id="section-step" type="number" step="1" value="${escapeHtml(d.step)}"></div><div class="field"><label>Link to Main Row Counter</label><select id="section-linked"><option value="yes" ${d.linked?"selected":""}>On</option><option value="no" ${!d.linked?"selected":""}>Off</option></select></div><div class="field"><label>Optional colour tag</label><input id="section-color" type="color" value="${escapeHtml(d.color)}"></div></section></div>`;document.getElementById("create-sub").innerHTML=mode==="repeatCounter"?'<span class="repeat-save-desktop">Save Repeat Rule</span><span class="repeat-save-mobile">Save rule</span>':'<span class="repeat-save-desktop">Save Sub-Counter</span><span class="repeat-save-mobile">Save counter</span>';host.querySelectorAll("input,select,textarea").forEach(element=>element.addEventListener("input",updatePreview));updatePreview();};
