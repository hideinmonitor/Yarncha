...
function renderSettingsSection(host) {
  ...
  section.className = "card mobile-card settings-panel settings-wide";
  section.innerHTML = `<p class="eyebrow">PRIVATE BETA CLOUD</p><h2>${cloud.user ? "Cloud account and sync" : "Sign in and cloud backup"}</h2><p>${cloud.configured ? (cloud.user ? `Signed in as <strong>${local().escapeHtml(cloud.user.email || "Yarncha user")}</strong>. Local drafts remain available offline.` : "Create an email account to keep private projects across devices.") : "Supabase environment variables have not been configured for this build."}</p><div class="storage-status-panel"><span>Local save: <strong>Enabled</strong></span><span>Cloud: <strong>${cloud.user ? "Connected" : cloud.configured ? "Sign-in required" : "Not configured"}</strong></span><span id="cloud-sync-status">${cloud.lastError || (cloud.user ? "Ready" : "Local-only")}</span></div><div class="button-row"><button class="primary-button" id="settings-cloud-account">${cloud.user ? "Account & sync" : "Sign in / create account"}</button>${cloud.user ? `<button class="secondary-button" id="settings-cloud-migrate">Move my local projects to cloud</button><button class="danger-button" id="settings-delete-account">Delete account and cloud data</button>` : ""}<a class="secondary-button button-link" href="mailto:feedback@yarncha.app?subject=Yarncha%20private%20beta%20feedback">Send beta feedback</a></div><div class="privacy-note"><strong>Privacy:</strong> Projects, charts and generated patterns are private to your account. Charts are sent to the configured server-side AI provider only when you press Analyse. AI output is a draft: unclear symbols remain uncertain and require your review.</div>`;
