...
async function loadTesseract(){if(window.Tesseract)return;await new Promise((resolve,reject)=>{const s=document.createElement("script");s.src="./vendor/tesseract/tesseract.min.js";s.onload=resolve;s.onerror=reject;document.head.appendChild(s);});}
async function ocrFile(file){
  try{
    await loadTesseract();
    const worker=await Tesseract.createWorker("eng+chi_tra+chi_sim+jpn",1,{workerPath:"./vendor/tesseract/worker.min.js",langPath:"./vendor/tesseract/lang",corePath:"./vendor/tesseract/core",logger:m=>{const p=getProject();if(m.status==="recognizing text"){p.readerStatus=`OCR ${file.name}: ${Math.round((m.progress||0)*100)}%`;const el=document.querySelector(".reader-status");if(el)el.innerHTML=`<strong>Reader status:</strong> ${escapeHtml(p.readerStatus)}`;}}});
    const outputs=[];
    if(file.type==="application/pdf"){
