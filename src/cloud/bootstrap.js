...
function openAccountModal() {
  ...
  if (cloud.user) {
    api.openModal(`<div class="account-menu-sheet"><p class="eyebrow">YARNCHA ACCOUNT</p><h2>${api.escapeHtml(cloud.user.email || "Signed in")}</h2><div class="sync-status"><strong id="cloud-sync-status">Cloud sync ready</strong><p>Local drafts stay on this device. Your owned cloud projects are protected by account-level database policies.</p></div><div class="account-destination-list"><button class="secondary-button" id="cloud-account-settings">Settings</button><button class="secondary-button" id="cloud-appearance-settings">Appearance</button></div><div class="modal-actions"><button class="secondary-button" id="cloud-migrate-now">Move my local projects to cloud</button><button class="secondary-button" id="cloud-refresh-now">Refresh from cloud</button><button class="secondary-button" id="cloud-sign-out">Sign out</button></div></div>`);
