...
assert.match(source, /Chart ready/, "Flow Mode shows a compact ready state");
assert.match(source, /Quick check needed/, "Flow Mode shows a compact review state");
assert.match(source, /READING PROGRESS/, "Flow Mode includes Reading Progress");
assert.match(source, /PROJECT DETAILS/, "Flow Mode includes a compact Project Guide");
...
assert.match(source, /<output id="flow-current-row" aria-live="polite">Row <strong>\$\{p\.row\}<\/strong><\/output>/, "current row is prominent read-only guidance");
assert.doesNotMatch(source, /<input id="flow-current-row"/, "current row does not look like an editable input");
...
assert.match(source, /Reading direction/, "row direction copy is concise");
assert.match(source, /class="flow-next-line"/, "next-row context is an inline message");
assert.doesNotMatch(source, /class="flow-next-card"/, "next-row context is not another card");
...
assert.match(styles, /\.flow-current-row-card output/, "large current row output is styled");
...
assert.match(styles, /\.flow-notice/, "setup warnings use a compact notice");
assert.match(styles, /\.result-summary\.result-summary-plain/, "Flow Mode uses the shared plain summary variant");
...
assert.equal(declarations('.result-summary.result-summary-plain')['border'],'0','Flow summary has no enclosing nested border');
assert.equal(declarations('.result-summary-plain .result-summary-row')['border'],'0','Flow summary rows are not boxed');
assert.equal(declarations('.result-summary-plain .result-summary-row')['border-bottom'],'1px solid var(--border)','Flow summary rows use dividers');
assert.equal(declarations('.flow-reader-panel')['background'],'transparent','secondary Flow panels are flat');
assert.equal(declarations('.flow-read-aloud')['border'],undefined,'Voice Assistant is not another bordered card');
