...
assert.match(app, /function configureServiceWorker\(\)[\s\S]*register\("\/service-worker\.js",\{scope:"\/",updateViaCache:"none"\}\)/, "Preview and hosted Yarncha both exercise the production service worker");
assert.match(app, /registration\.addEventListener\("updatefound"[\s\S]*await registration\.update\(\)/, "Service-worker updates are checked and reported");
